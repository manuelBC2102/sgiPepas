...
$stmt = $mysqli->prepare("UPDATE cajas_chicas_lote SET ccl_monto_usado=?, ccl_ret=?, ccl_ret_no=?, ccl_det=?, ccl_det_no=?, est_id=?, ccl_act_fec=?, ccl_act_usu=?, ccl_gti=?, ccl_dg_json=?, ccl_gast_asum=?,
	ccl_pend=?, ccl_cuadre=?, ccl_banco=?, ccl_aju=? WHERE ccl_id=?") or die ($mysqli->error);
$stmt->bind_param('dddddisiisdddddi',
	$tot_mon_liq,
	$tot_mon_ret,
	$tot_mon_ret_no,
	$tot_mon_det,
...
if ($est_id==3) {
	$stmt = $mysqli->prepare("DELETE FROM cajas_chicas_lote_det_hist WHERE ccl_id=? AND hist_id=2") or die ($mysqli->error);
	$stmt->bind_param('i',
		$id);
	$stmt->execute() or die ($mysqli->error);
}
...
if ($est_id == 3) {
	$stmt = $mysqli->prepare("INSERT INTO cajas_chicas_lote_act VALUES (?, 3, ?, ?, null)") or die ($mysqli->error);
	$stmt->bind_param('iis', $id, $_SESSION['rec_usu_id'], $ahora);
	$stmt->execute() or die ($mysqli->error);

	$stmt = $mysqli->prepare("UPDATE cajas_chicas_lote SET ccl_desemb=?, ccl_aprob_fec=?, ccl_aprob_usu=? WHERE ccl_id=?") or die ($mysqli->error);
	$stmt->bind_param('dsii', $tot_mon_liq, $ahora, $_SESSION['rec_usu_id'], $id);
	$stmt->execute() or die ($mysqli->error);
}
else {
	$stmt = $mysqli->prepare("INSERT INTO cajas_chicas_lote_act VALUES (?, 62, ?, ?, null)") or die ($mysqli->error);
	$stmt->bind_param('iis', $id, $_SESSION['rec_usu_id'], $ahora);
	$stmt->execute() or die ($mysqli->error);
}
...
if ($est_id == 3) {
	...
	$cc = array();
	array_push ($cc, getCorreoUsuario($ccl_cie_usu));
	...
	$subject = "Lote Aprobado de CAJA CHICA $ccl_numero";
	$body = "Se ha aprobado el lote de la CAJA CHICA $ccl_numero de $cch_nombre realizado por ".getNombreTrabajador(getCodigoGeneral(getUsuAd($usu_id))).".";
	$espc = $body;
	...
	$espc 	  .= "\n\nFavor de ingresar al modulo Administracion de la web intranet y realizar el desembolso respectivo.";
	$ventana   = "admin/cch_lote_desemb.php";
	$aprobador = getUsuAd(getUsuTesoreria());
	$detalle   = ConstructorMail::armarDetalle(null, $subject, $espc, $attachString, $attachFilename);
	ConstructorMail::enviarCorreoInfoConLink($id, $aprobador, $ventana, $var_modulo_ident, $detalle);


	$to = null;
	$cc = array();
	array_push ($cc, getUsuAd(getUsuSupCont()));
	array_push ($cc, getUsuAd(getUsuRegCont()));
	array_push ($cc, getUsuAd(getUsuAnaCont()));
	$subject = "Lote Aprobado de CAJA CHICA $ccl_numero";
	$body = "Se ha aprobado el lote de la CAJA CHICA $ccl_numero de $cch_nombre realizado por ".getNombreTrabajador(getCodigoGeneral(getUsuAd($usu_id))).".";
	$body .= "\n\nEsta pendiente la revision del lote de CAJA CHICA, favor de ingresar al modulo Administracion de la web intranet, opcion Caja Chica y realizar los ajustes necesarios y dar el visto bueno respectivo.";
	//R. enviarCorreo($to, $cc, $subject, $body, $attachString, $attachFilename);
	$ventana   = "admin/cch_contabilidad.php";
	$aprobador = $cc;
...
header("Location: cch_lote_revision_res.php?id=$id&o=$oper");
exit;
?>
