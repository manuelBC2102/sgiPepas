...
<head>
<title>Mantenedor Gastos Colobjects (GCO COBJ) - Administraci�n MinappES</title>
<style type="text/css">
...
<script>
function editar(id) {
	location.href="mant_colobjects_edit.php?id="+id;
}

function borrar(id, desc, cod, mon) {
	var valor=confirm('Est\u00e1 seguro que desea eliminar el colobjects '+desc+' ('+cod+') '+mon+'?');

	if(valor==true) { location.href="mant_colobjects_del_p.php?id="+id; }
}
</script>
<style>
.iconos {
...
<h1>Mantenedor Gastos Colobjects (GCO COBJ) <a href="mant_colobjects_new.php"><img src="img/new_icon24.png" border="0" title="Registrar nuevo colobjects"></a></h1>
...
foreach ($arr as $v) {
	...
	echo "\t\t\t<img src='img/delete.png' border='0' title='Borrar' class='iconos' onclick='borrar(\"$v[0]\", \"$v[2]\", \"$v[1]\", \"$v[3]\");'>\n";
	echo "\t\t</td>\n";
