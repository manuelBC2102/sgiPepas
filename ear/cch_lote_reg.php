...
$arr = getUltLoteCajaChica($id);
// Si no existen lotes en esa caja chica se crea un nuevo lote
if (count($arr) == 0) {
	echo nuevoLoteCajaChica($id, $usu_id);
	$arr = getUltLoteCajaChica($id);
}
else {
	// Si el ultimo lote de esa caja chica esta en estado desembolsado se crea un nuevo lote
	if (in_array($arr[20], array(3, 5, 6, 7)) && $arr[33]==1) {
		echo nuevoLoteCajaChica($id, $usu_id, $arr[0]);
		$arr = getUltLoteCajaChica($id);
	}
	// Si el ultimo lote de esa caja chica esta en un estado diferente a aperturada no se puede continuar
	else if($arr[20]!=1) {
		echo "<font color='red'><b>ERROR: Caja chica esta cerrada, esperando desembolso.</b></font><br>";
		exit;
	}
}
...
<title>Registrar Documentos Caja Chica - Administraci�n - Minapp</title>
<style type="text/css">
...
$(document).ready(function()
{
	...
	$('#doc_sust_detalle').on('click', '.pers_cla', function(){
		// dg_header = 1;
		$( "#pers_tab" ).data('dist_gast_node', $(this).parent().parent() ).dialog( "open" );
		// $('#tg').change();
	});
	...
	$( "#pers_tab" ).dialog({
		...
		buttons: {
			"Seleccionar": function() {
				var dni = $('#pers_sel option:selected').val();
				$( "#pers_tab" ).data('dist_gast_node').children('.ruc_nro_td').children('.ruc_nro_i').attr('value', dni);
				$( "#pers_tab" ).data('dist_gast_node').children('.ruc_nro_td').children('.ruc_nro_i').change();
	...
	$('#doc_sust_detalle').on('change', '.tipo_doc', function(){
		...
		if (parseInt($(this).val()) == 10) {
			...
			$(this).parent().children('.pers_cla').show();
		}
		else {
			...
			$(this).parent().children('.pers_cla').hide();
		}
	...
	$(document).on('focus',".fecha_inp", function(){
		$(this).datepicker({
			numberOfMonths: 2,
			maxDate: 0
		});
	...
	$.fn.totalizar_recalc = function() {
		...
		var mon_abodes = tot_mon_liq_res;
		var resul_msg = "";
		var resul_inp = 0;
		switch (true) {
			case (mon_abodes == 0):
		...
		var tot_cuadre_cch = tot_mon_liq+tot_doc_pend+tot_cuadre+tot_mon_banco;
	...
	$(document).ready(function() {
		...
		$('.afecto_inp').each(function(){
			...
			var tipo_doc = $(this).parent().parent().children('.tipo_doc_td').children('.tipo_doc').val();
			if (tipo_doc == 10) {
				$(this).parent().parent().children('.tipo_doc_td').children('.pers_cla').show();
			}
		});
	...
	var pers_ico = '<img src="img/modal.gif" class="pers_cla" title="Abrir selector de personal" style="display: none;">';
	...
	$('#bole_add').click(function(){
		...
		s_add += '<td class="tipo_doc_td">'+tipo_doc_t+' '+pers_ico+'</td>'; //Tipo Doc
		...
		s_add += '<td>'+dist_gast+'</td><td>'+dele+'</td>';
		s_add += '</tr>';
	...
	$('#enviar').click(function( event ){
		...
		$('#liquidacion .prov_act').each(function(){
			...
			else if (ruc_act == -1 && tipo_doc == 10) {
				alert('ERROR: Existen recibos de gastos sin DNI ingresado, no se puede continuar.\nVerifique que este seleccionado correctamente el tipo de documento.');
				saltar =1 ;
				return false; // Solo sale del loop each
			}
		});
		...
		if (total_liq == 0) {
			var valor=confirm('ALERTA: El monto del total registrado es cero. Est\u00e1 seguro de continuar?.');
			if (valor==false) {
				return false;
			}
		}

		if (!confirm('Est\u00e1 seguro de enviar el lote?')) {
			return false;
		}
	});
...
<style>
#tg1, #tg2, #tg3, #tg4, #tipo_doc_template, #tipo_mon_template, #ruc_nro_template, #dist_gastos, #pers_tab {
	display:none;
}
...
<h1>Registrar Documentos Caja Chica</h1>
...
<form id="liquidacion" action="cch_lote_reg_p.php" method="post">
...
<tbody id="bole_body">
<tr>
	<td colspan="19">Boletos de Viaje / Pasajes A�reos</td>
</tr>
<tr>
...
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '01', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Boletos de Viaje / Pasajes A�reos <img src="img/plus.png" id="bole_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
...
<tbody id="alim_body">
<tr>
	<td colspan="19">Alimentacion / Pension</td>
</tr>
<tr>
	<td class="encabezado_h">Concepto</td>
	<td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
...
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '02', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Alimentacion / Pension <img src="img/plus.png" id="alim_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
...
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '03', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Hospedaje <img src="img/plus.png" id="hosp_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
...
<tbody id="movi_body">
<tr>
	<td colspan="19">Movilidad / Combustible</td>
</tr>
<tr>
	<td class="encabezado_h">Concepto</td>
	<td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
...
<?php echo getFilaPlanillaMovilidadCCH($ccl_id, $mon_id, 1, $_SESSION['rec_usu_nombre'], $_SESSION['adm_usu_gco_cobj']); ?>
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '04', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Movilidad / Combustible <img src="img/plus.png" id="movi_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
...
<tbody id="gast_body">
<tr>
	<td colspan="19">Gastos de Representaci�n</td>
</tr>
<tr>
	<td class="encabezado_h">Concepto</td>
	<td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
...
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '05', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Gastos de Representaci�n <img src="img/plus.png" id="gast_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
...
<?php echo getFilasPrevias($arrLiqDet, $mon_id, '06', 1); ?>
</tbody>
<tbody>
<tr>
	<td colspan="19">Nueva fila Otros <img src="img/plus.png" id="otro_add" title="Agregar" class="iconos"></td>
</tr>
</tbody>
</table>
...
<?php echo getFilasPreviasDocPend($ccl_id, 1); ?>
</tbody>
<tbody>
<tr>
...
<input type="hidden" value="<?php echo $ccl_id; ?>" name="id">
...
<input type="submit" value="Grabar Lote" name="grabar" id="grabar" disabled> <input type="submit" value="Cerrar y Enviar Lote" name="enviar" id="enviar" disabled>

<p>Descripci�n de los botones:<br>
<b>Grabar</b> almacena los datos ingresados para continuar despu�s.<br>
<b>Cerrar y enviar</b> transmite el lote al responsable para su revisi�n y aprobaci�n (Ya no podr� hacer m�s modificaciones). </p>
...
<div id="pers_tab" title="Selector de Personal">
	<table>
	<tr>
		<td>Nombres: </td>
		<td>
			<select id="pers_sel">
<?php
$arr = getListaTrabajadores();

foreach ($arr as $v) {
	echo "\t\t\t\t<option value='$v[0]'>$v[1] $v[2] $v[3]</option>\n";
}
?>
			</select>
		</td>
