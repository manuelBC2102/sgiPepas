...
$stmt = $mysqli->prepare("INSERT INTO ear_actualizaciones VALUES (?, 1, ?, ?, 'EAR Cero por otro usuario')") or die ($mysqli->error);
$stmt->bind_param('iis', $insertion_id, $_SESSION['rec_usu_id'], $ahora);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("INSERT INTO ear_actualizaciones VALUES (?, 2, ?, ?, 'EAR Cero por otro usuario')") or die ($mysqli->error);
$stmt->bind_param('iis', $insertion_id, $_SESSION['rec_usu_id'], $ahora);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("INSERT INTO ear_actualizaciones VALUES (?, 4, ?, ?, 'EAR Cero por otro usuario')") or die ($mysqli->error);
$stmt->bind_param('iis', $insertion_id, $_SESSION['rec_usu_id'], $ahora);
$stmt->execute() or die ($mysqli->error);
...
$desc = "Registro solicitud EAR Cero por otro usuario (".$insertion_id.") de ".$_SESSION['rec_usu_nombre'];
...
$subject = "Solicitud Registrada de EAR Cero $ear_numero de ".$nombres." por ".getNombreTrabajador($_SESSION['rec_codigogeneral_id']);
