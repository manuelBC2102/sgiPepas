...
$personaIdInicial=$arrayUsuario[0]['id'];
	...
	$(document).ready(function() {
		var oTable = $('#example').dataTable( {
            ...
            $('#slave_usu_id').on('change', function(){
                ...
                location.href="ear_liquidacion_otro.php?usuId="+usuId;
            });
...
<?php include ("header.php");
    ...
    if ($nombresUsuario == '') {
        $nombresUsuario = $arrayUsuario[0]['persona_nombre'];
    }
?>

<h1>Registro de liquidaci&oacute;n <span id="nombreUsuario">(<?php echo strtoupper($nombresUsuario); ?>)</span></h1>
...
<div class="full_width" style="margin-top: 1em; margin-bottom: 1em; padding-left: 1em; padding-right: 1em; width: auto;">
    <table>
        <tr>
            ...
            <td>
				<select id="slave_usu_id" class="select2" >
                    <?php
            		$listOrdenTrabajo='';
            		$listOrdenTrabajo .= '<option value="">Seleccione un Colaborador</option>';
                    foreach ($arrayUsuario as $item) {
                        $selected = '';
                        if ($item['id'] == $_GET["usuId"]) {
                            $selected = 'selected';
                        }
						$listOrdenTrabajo .= '<option value="' . $item['id'] . '" '.$selected.'>' . $item['codigo_identificacion'] . ' | ' . $item['persona_nombre'] . '</option>';
                    }
...
<?php
if ($opc_id == 10) {
	$q_usu = "(e.master_usu_id=".$usu_id." OR e.usu_id IN (".implode(",", getUsuRegOtroSlavesIds($usu_id))."))";
}
else {
	$q_usu = 'e.usu_id='.$usu_id;
}
