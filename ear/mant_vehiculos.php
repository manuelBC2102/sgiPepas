...
<head>
<!--<title>Mantenedor Vehiculos - Administraci�n - Minapp</title>-->
...
<script>
function editar(id) {
	location.href="mant_vehiculos_edit.php?id="+id;
}

function borrar(id, var01) {
	var valor=confirm('Est\u00e1 seguro que desea eliminar la marca de vehiculo '+var01+'?');

	if(valor==true) { location.href="mant_vehiculos_del_p.php?id="+id; }
}
</script>
<style>
.iconos {
...
<h1>Mantenedor de veh&iacute;culos<a href="mant_vehiculos_new.php"><img src="img/new_icon24.png" border="0" title="Registrar nueva marca de vehiculo"></a></h1>
...
foreach ($arr as $v) {
	...
	echo "\t\t<td>".$v[2]."</td>\n";
	echo "\t\t<td>".$v[3]."</td>\n";
	echo "\t\t<td>".(is_null($v[4])?"Ninguno":$v[4])."</td>\n";
	echo "\t\t<td>".getSino($v[5])."</td>\n";
