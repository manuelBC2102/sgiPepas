...
$stmt = $mysqli->prepare("UPDATE tipo_cambio SET tc_precio=? WHERE tc_id=?") or die ($mysqli->error);
$stmt->bind_param('di',
	$pre,
	$id);
...
$desc = "Editar tipo de cambio ($id), fecha ($fec), nuevo valor ($pre), hecha por ".$_SESSION['rec_usu_nombre'];
