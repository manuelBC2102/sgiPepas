...
$tituloReporte='Reporte de entregas a rendir';
if ($cons_id == 1) {
    $tituloReporte='Reporte y edici&oacute;n de solicitudes ';
}else if($cons_id == 2) {
    $tituloReporte='Aprobaci&oacute;n de solicitudes';
}else if($cons_id == 5) {
    $tituloReporte='Consulta de liquidaciones descontadas';
}
...
//	$count += getPermisosAdministrativos($_SESSION['rec_usu_id'], 'ANA_CONT');
	...
	if ($opc_id == 1) {
                $tituloReporte='Consulta de solicitudes desembolsadas';
		$rpt_nom = "Solicitudes desembolsadas";
		$col8 = "Fecha desemb";
	}
	else if ($opc_id == 2) {
                $tituloReporte='Consulta de liquidaciones reembolsadas';
...
<!--<title>Reporte de Entregas a Rendir (EAR) - Administraci�n - Minapp</title>-->
<style type="text/css">
...
function aprobarSolicitud(earId){
    var msj = 'Esta seguro que desea aprobar la solicitud?';
    ...
    location.href="ear_sol_vistobueno_p.php?id="+earId+"&opcion=2";
}
...
<form method="post" action="ear_consulta_p.php">
...
- Estado
<?php
    $dataEstado=tablasEstadosObtenerComboXTablaId(1);
    $htmlEstado='<select name="estado">';
    foreach ($dataEstado as $item){
        $selected='';
        if($est_id==$item['est_id']) $selected= 'selected';
        $htmlEstado.='<option value="'.$item['est_id'].'"'.$selected.' >'.$item['est_nom'].'</option>';
    }
    //para todos
    $selected = '';
    if ($est_id == 255)
        $selected = 'selected';
    $htmlEstado.='<option value="255"' . $selected . ' >Todos</option>';
...
?>
	...
	<tbody>
<?php
if ($cons_id == 3 && $opc_id == 1) {
	$arr = getListaEARsDescargaExcelLiq($max, $cons_id, $_SESSION['rec_usu_id'], $zon_id, $mon_id, $est_id, $rfecha2, $rfecha4, $opc_id);
}
else {
...
foreach ($arr as $v) {
	...
	echo "\t\t<td>".$v[2]."</td>\n";
	echo "\t\t<td ".(strlen($v[20])>$longMotivo?"title='".$v[20]."'":"").">".obtenerCadenaXNumLetras($v[20],$longMotivo)."</td>\n";
	...
	if (($cons_id == 1 && $opc_id == 2) || ($cons_id == 1 && $opc_id == 12)) {
		...
		echo "\t\t<td align='right'>".conComas($v[19]*-1)."</td>\n";
	}
	else if ($cons_id == 5) {
		echo "\t\t<td align='right'>".conComas($v[19])."</td>\n";
	}
	else {
...
//                        echo "\t\t<td>".$v[15]."</td>\n";
	...
	if ($cons_id==2 && $v[8]==1) {
		echo "<a style='cursor: pointer' onClick=\"aprobarSolicitud('".$v[0]."')\"><img src='img/opc-si.gif' title='Aprobar' border='0' class='iconos'></a> ";
	...
	if ($v[8]==1 || $v[8]==3 || $v[8]==41) {
		...
		echo "<a href='ear_pdf_sol.php?id=".$v[0]."' target='_blank'><img src='img/pdf.gif' border='0' title='Descargar PDF Solicitud EAR' class='iconos'></a>\n";
	}

	// Columna fija del icono de visualizar y la descarga PDF de la liquidacion EAR
	if ( ($v[8]>=5 && $v[8]<=12) || ($v[8]>=51 && $v[8]<=53) ) {
		echo "<a href='ear_liq_consulta.php?id=".$v[0]."'><img src='img/search.png' border='0' title='Visualizar la Liquidaci&oacute;n EAR' class='iconos'></a>\n";
		echo "<a href='ear_pdf_liq.php?id=".$v[0]."' target='_blank'><img src='img/pdf.gif' border='0' title='Descargar PDF Liquidacion EAR' class='iconos'></a>\n";
	}
	else {
		echo "<img src='img/transparent.gif' border='0' class='iconos'>\n";
		echo "<img src='img/transparent.gif' border='0' class='iconos'>\n";
	...
	if ($cons_id==3) {
		if ($opc_id==1) {
			echo "<span class='ear_excel_liq' fila='".$v[0]."' style='cursor: pointer'><img src='".($v[19] > 0?'img/l.png':'img/transparent.gif')."' border='0' title='Descargar Excel Liquidacion' class='iconos'></span>\n";
			echo "<a href='ear_excel_ret.php?id=".$v[0]."'><img src='".($v[20] > 0?'img/r.png':'img/transparent.gif')."' border='0' title='Descargar Excel Retenciones' class='iconos'></a>\n";
			echo "<a href='ear_excel_det.php?id=".$v[0]."'><img src='".($v[21] > 0?'img/d.png':'img/transparent.gif')."' border='0' title='Descargar Excel Detracciones' class='iconos'></a>\n";
			echo "<a href='ear_excel_acf.php?id=".$v[0]."'><img src='".($v[22] > 0?'img/a.png':'img/transparent.gif')."' border='0' title='Descargar Excel Activo Fijo' class='iconos'></a>\n";
			echo "<a href='ear_excel_aju.php?id=".$v[0]."'><img src='".($v[23] > 0?'img/aj.jpg':'img/transparent.gif')."' border='0' title='Descargar Excel Ajustes' class='iconos'></a>\n";
		}
	}
...
<b>Nota:</b> M&aacute;ximo se muestran <?php echo $max; ?> registros por consulta web, la exportaci&oacute;n a hoja excel no tiene l&iacute;mites de registros por consulta.<br>
</div>
