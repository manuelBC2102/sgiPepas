...
if (isset($f_conc_l)) {
	foreach ($f_conc_l as $k => $v) {
		...
		if ($est_id==6 && $doc_id==10) {
			...
			$stmt->bind_param('iiisssssiidddiidissid',
				$id,
				$conc_id,
				$doc_id,
				$ruc_nro,
...
if ($est_id == 6) {
$motivo_act = 'Hecha por: '.getUsuarioNombre($_SESSION['rec_usu_id']).'.';
$stmt = $mysqli->prepare("INSERT INTO ear_actualizaciones VALUES (?, 6, ?, ?, ?)") or die ($mysqli->error);
$stmt->bind_param('iiss', $id, $otro_jefe_usu_id, $ahora, $motivo_act);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("UPDATE ear_solicitudes SET ear_act_usu=? WHERE ear_id=?") or die ($mysqli->error);
$stmt->bind_param('ii', $otro_jefe_usu_id, $id);
$stmt->execute() or die ($mysqli->error);
}
...
if ($est_id == 6) {
	...
	$body .= "\n\nNota al colaborador: Imprimir, firmar y hacer entregar a la brevedad el documento PDF adjunto a Administracion con los documentos fisicos sustentatorios de la liquidacion, gracias.";
	$body .= "\n\nEsperando el visto bueno de la Administracion.";
	...
	$body = "Se ha aprobado la liquidacion del EAR $ear_numero de $ear_tra_nombres.";
	$body .= "\n\nEsta aprobacion fue realizada por ".getUsuarioNombre($_SESSION['rec_usu_id'])." en vez de ".getUsuarioNombre($usu_id_jefe).".";
	$body .= "\n\nFavor de ingresar al modulo Administracion de la web intranet, opcion Visto Bueno de Liquidaciones y realizar los ajustes necesarios y dar el visto bueno respectivo.";
	enviarCorreo($to, $cc, $subject, $body, $attachString, $attachFilename);
}

include 'datos_cerrar_bd.php';

header("Location: oper_otro_jefe_ear_liq_revision_res.php?id=$id&o=$oper&otro_jefe_usu_id=$otro_jefe_usu_id");
exit;
?>
