...
$totalEAR = number_format($ear_liq_mon, 2, '.', '');
...
<head>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      //deshabilito todo
      $('#liquidacion input').prop('disabled', true);
      $('#liquidacion select').prop('disabled', true);

      //habilito algunas columnas
      $('#liquidacion .tipo_doc').removeAttr("disabled");
      $('#liquidacion .ser_doc_inp').removeAttr("disabled");
      $('#liquidacion .num_doc_inp').removeAttr("disabled");
      $('#liquidacion .afecto_sel').removeAttr("disabled");
      $('#liquidacion .afecto_inp').removeAttr("disabled");
      $('#liquidacion .noafecto_inp').removeAttr("disabled");
      $('#liquidacion .montOtro_inp').removeAttr("disabled");
      $('#liquidacion .montIcbp_inp').removeAttr("disabled");
      $('#liquidacion .fecha_inp').removeAttr("disabled");
      $('#liquidacion .tipo_mon').removeAttr("disabled");
      $('#liquidacion .tc_inp').removeAttr("disabled");
      $('#liquidacion .doc_deducible').removeAttr("disabled");

      //HABILITO TIPO DE CAMBIO Y DUA
      //	$('#liquidacion #txtTipoCambioLiq').removeAttr("disabled");
      //	$('#liquidacion #chkTipoCambio').removeAttr("disabled");
      //	$('#liquidacion #dua').removeAttr("disabled");
      $('#liquidacion #periodoId').removeAttr("disabled");
      ...
      $.fn.checktopes_redraw = function() {
        ...
        $('#alim_body .fecha_inp').each(function() {
          ...
          icbp = parseFloat(fila.children('.conv_icbp_td').children('.conv_icbp_inp').val()) || 0;
          monto = afecto + noafecto + otro + icbp;
      ...
      $.fn.plamov_redraw = function() {
        ...
        $.when(
          $.getJSON('ws_pla_mov_info.php', {
            id: id
          }, function(data) {
            $('#pla_monto_div').html(data.pla_monto);
            if (data.est_id == 3) {
              $('#pm_estado').html("Falta revisar <a href='movi_consulta_detalle.php?id=" + id + "&opc=3' target='_blank'><img src='img/modal.gif' id='abrir_plamov' class='iconos' title='Abrir Planilla de Movilidad en una nueva ventana'></a> <img src='img/reload.gif' id='act_est_plamov' class='iconos' title='Actualizar estado'>");
            } else if (data.est_id == 4) {
              $('#pm_estado').html("Revisado <a href='movi_consulta_detalle.php?id=" + id + "&opc=3' target='_blank'><img src='img/modal.gif' id='abrir_plamov' class='iconos' title='Abrir Planilla de Movilidad en una nueva ventana'></a> <img src='img/reload.gif' id='act_est_plamov' class='iconos' title='Actualizar estado'>");
            } else if (data.est_id == 5) {
              $('#pm_estado').html("Revisado VB <a href='movi_consulta_detalle.php?id=" + id + "&opc=3' target='_blank'><img src='img/modal.gif' id='abrir_plamov' class='iconos' title='Abrir Planilla de Movilidad en una nueva ventana'></a> <img src='img/reload.gif' id='act_est_plamov' class='iconos' title='Actualizar estado'>");
            } else {
      ...
      $('#doc_sust_detalle').on('change', '.tipo_doc', function() {
        ...
        if (apliDistribucion == 0) {
          ...
          $("input[name='lid_operacion_tipo" + numeroFila + "']").val("");
      ...
      $.fn.totalizar_recalc = function() {
        ...
        $('.conv_afecto_inp').each(function() {
          ...
          if (tipo_mon_fila == 2 && mon_id == 1) {
            ...
            mon_gast_asum_fila = (factor * (mon_gast_asum_fila * tc_fila)).toFixed(2);
          ...
          tot_mon_gast_asum += parseFloat(factor * mon_gast_asum_fila);
        });

        var tot_mon_liq_res = redondearXDecimales(tot_mon_liq - tot_mon_ret - tot_mon_det, 6);
        ...
        var tot_mon_gast_asum2 = redondearXDecimales(tot_mon_liq - tot_mon_gast_asum, 6); // Gastos asumidos por el colaborador
      ...
      $.fn.gast_asum_recalc = function(fila) {
        ...
        var monto_icbp = (parseFloat(fila.children('.montIcbp_td').children('.montIcbp_inp').val())) || 0;
        ...
        var taxcode = parseInt(fila.children('.tipo_doc_td').children('.tipo_doc').children('option:selected').attr('taxcode'));
          if(taxcode == 6){
            fila.children('.noafecto_td').children('.noafecto_inp').valid();
          }
      };
      ...
      $.fn.tipo_notaf = function() {
        ...
        // $("input[name='lid_operacion_tipo" + numeroFila + "']").val("");
        var agregarDistribucion = '&nbsp;<input type="hidden" value=""  id="lid_distribucion[{0}]" name="lid_distribucion[{0}]" /><input type="hidden" value=""  id="lid_operacion_tipo[{0}]" name="lid_operacion_tipo[{0}]" /><img  id="op_distribucion[{0}]" name="op_distribucion[{0}]" class="op_distribucion" src="img/plus.png"  onclick="abrirModalDistribucion(\'[{0}]\',\'' + IdenBody + '\')" title="Agregar distribucion contable">&nbsp;';
        ...
        s_add += '<td>' + aprob_t + '</td>';
        s_add += '<td class="gast_asum_td">' + gast_asum_t + '</td>';
        s_add += '<td class="doc_deducible_td"><select id="doc_deducible[0]" class="doc_deducible" name="doc_deducible[0]"><option value="1" selected="">Si</option><option value="0">No</option></select></td>'; //Deducible
        s_add += '<td><input type="hidden" value="' + documento_relacion + '" id="documento_relacion[{0}]" name="documento_relacion[{0}]"/> ' + agregarDistribucion + dele + '</td>';
        s_add += '</tr>';
      ...
      s_afeconretdet += '<td class="tc_td"><div class="tc_div" style="display: none;"></div><input size="6" maxlength="9" class="tc_inp" name="tc_inp[{0}]" value="1"></td>'; //T/C;
      ...
      s_afeconretdet += '<td class="conv_icbp_td"><input type="text" value="" size="8" maxlength="9" id="conv_icbp_inp[{0}]" class="conv_icbp_inp" name="conv_icbp_inp[{0}]" style="display: none;" readonly></td>'; //Conversion IBCP
      ...
      $('#bole_add').click(function() {
        ...
        var gast_asum_t = $('#gast_asum_template').html();
        var dele = '<img src="img/delete.png" class="dele" title="Borrar">';
        var agregarDistribucion = '&nbsp;<input type="hidden" value=""  id="lid_distribucion[{0}]" name="lid_distribucion[{0}]" /><input type="hidden" value=""  id="lid_operacion_tipo[{0}]" name="lid_operacion_tipo[{0}]" /><img src="img/plus.png"  onclick="abrirModalDistribucion(\'[{0}]\',\'01\')" title="Agregar distribucion contable">&nbsp;';
        ...
        s_add += '<td class="aprob_sel_td">' + aprob_t + '</td>';
        s_add += '<td class="gast_asum_td">' + gast_asum_t + '</td>';
      ...
      $('#aprobar').click(function(event) {
        ...
        if (!confirm('Est\u00e1 seguro de aprobar la liquidacion?')) {
          return false;
        }
      });
      ...
      //VALIDAMOS QUE EL IMPORTE INICIAL SEA IGUAL AL MODIFICADO
      $('#grabar').on('click', function() {
        ...
        var totalLiq = $('#tot_mon_liq').val();
        var totalLiqAntes = <?php echo $totalEAR; ?>;

        if (totalLiq * 1 != totalLiqAntes * 1) {
          alert('El monto liquidado debe ser igual al monto inicial liquidado: ' + totalLiqAntes.toFixed(2));
          return false;
        }
        var saltar = -1;
        /*
        ...
        $("#doc_sust_detalle .conc_l").each(function () {
          var id = $(this).attr("id");
          id = id.replace("conc_l", "");
          var operacion_tipo = $("input[name='lid_operacion_tipo" + id + "']").val();
          var combo = document.getElementById("tipo_doc" + id);
          var nombre_documento = combo.options[combo.selectedIndex].text;
          if (combo.options[combo.selectedIndex].getAttribute('apldistri') == '0') {
            return true;
          }
          var serie_documento = $("input[name='ser_doc" + id + "']").val();
          var correlativo_documento = $("input[name='num_doc" + id + "']").val();
          ...
          var doc = combo.options[combo.selectedIndex].value;
          let comboDocumentoTipo =  combo.options[combo.selectedIndex].getAttribute("taxcode");
          if(comboDocumentoTipo == "3" || comboDocumentoTipo == "4"  || (comboDocumentoTipo == "6" && doc == "3") || (comboDocumentoTipo == "6" && doc == "4")){
            if (isEmpty(operacion_tipo) || operacion_tipo == "0") {
              alert('Aun no selecciona Operación Tipo  para ' + documento_seleccionado);
              saltar = 1;
              return false;
            }
          }
        ...
        //            if (duaId*1 == -1){
        //                alert('ERROR: Debe seleccionar una DUA valida.');
        //                return false;
        //            }

        //            var tipoCambioLiq = document.getElementById("txtTipoCambioLiq").value;
        //            if (tipoCambioLiq == ''){
        //                alert('ERROR: Debe ingresar tipo de cambio.');
        //                return false;
        //            }
        if (!confirm('Est\u00e1 seguro de actualizar la liquidacion?')) {
          return false;
        } else {
          //SI ACEPTA VOLVEMOS A HABILITAR TODO.
          $('#liquidacion input').removeAttr("disabled");
          $('#liquidacion select').removeAttr("disabled");
        }
    ...
    var dist_fila = null;
    var dist_carga_inicial = false;
    var dist_tipo = null;
    var dist_contOperacionTipoId = null;
    var dist_documentoTipoSgiId = null;
    var dist_dataContOperacionTipo = null;
    var dist_dataCuentaContable = null;
    var dist_distribucionContable = null;

    function abrirModalDistribucion(fila, tipo) {
      let comboDocumentoTipo = document.getElementById("tipo_doc" + fila);
      let documentoTipoId = document.getElementById("tipo_doc" + fila).value;
      if (isEmpty(documentoTipoId)) {
        alert('Primero debe seleccionar el tipo de documento.');
        return;
      }
      ...
      dist_carga_inicial = false;
      dist_documentoTipoSgiId = comboDocumentoTipo.options[comboDocumentoTipo.selectedIndex].getAttribute('sgiDocumentoId');

      dist_contOperacionTipoId = document.getElementById("lid_operacion_tipo" + fila).value;
      ...
      $.getJSON('fun_contable.php', {
        accion: "OPXDT",
        documentoTipoId: dist_documentoTipoSgiId
      }, function(data) {
        dist_dataContOperacionTipo = data;
        let htmlSelectOperacionTipo = "<select class='cont_operacion_tipo' id='cboContOperacionTipo' name='cboContOperacionTipo' onchange='onChangeCboOperacionTipo(this.value);'>";

        $.each(data, function(index, item) {
          htmlSelectOperacionTipo = htmlSelectOperacionTipo + "<option value='" + item.id + "' requiere_centro_costo='" + item.requiere_centro_costo + "'>" + item.descripcion + " </option>"
        });
        htmlSelectOperacionTipo = htmlSelectOperacionTipo + "</select>";

        document.querySelector('#modalDistribucion').style.display = 'block';
        $("#tdContOperacionTipoId").html(htmlSelectOperacionTipo);
        document.getElementById("cboContOperacionTipo").value = dist_contOperacionTipoId;
        onChangeCboOperacionTipo(dist_contOperacionTipoId)
      });
    }

    function onChangeCboOperacionTipo(id) {
      if (!isEmpty(id)) {
        $.getJSON('fun_contable.php', {
          accion: "CCXOP",
          operacionTipoId: id
          ...
          if (!isEmpty(data)) {
            let htmlSelectOperacionTipo = '<select class="cuenta_contable" id="cuenta_contable[{0}]"  name="cuenta_contable[{0}]">';

            $.each(data, function(index, item) {
              htmlSelectOperacionTipo = htmlSelectOperacionTipo + "<option value='" + item.id + "' " + (item.esPadre == 1 ? "disabled" : "") + ">" + item.descripcion + " </option>"
            });
            htmlSelectOperacionTipo = htmlSelectOperacionTipo + "</select>";
            $("#cuenta_contable_template").html(htmlSelectOperacionTipo);

            $("#distribucion_body tr.fila_dato").remove();

            if (dist_carga_inicial == false) {
              dist_carga_inicial = true;
              cargarDistribucion(dist_fila);
            }
          }
        });
      }
    ...
    function obtenerMontoTotalesXFila(fila) {
      ...
      var deducible = $("select[name='doc_deducible" + fila + "']").val();
      var montootro = $("input[name='montOtro_inp" + fila + "']").val();
      var montoibcp = $("input[name='montIcbp_inp" + fila + "']").val();

      const inputIGV = document.getElementById(`afecto_sel${fila}`).value;
      const cantIGV = 1 + ((inputIGV == '2' || inputIGV == '5') ? 0.10 : 0.18);

      montoafecto = (!isEmpty(montoafecto) ? (deducible == 1 ? (montoafecto * 1) / cantIGV : (montoafecto * 1)) : 0);
      montonoafecto = (!isEmpty(montonoafecto) ? montonoafecto * 1 : 0);
      montootro = (!isEmpty(montootro) ? montootro * 1 : 0);
      montoibcp = (!isEmpty(montoibcp) ? montoibcp * 1 : 0);
      return redondearDosDecimales(montoafecto + montonoafecto + montootro + montoibcp) * 1;
    }

    function obtenerTipoOperacionValidarCentroCosto() {
      let dataCombo = document.getElementById("cboContOperacionTipo");
      if (isEmpty(dataCombo)) {
        return;
      }
      return dataCombo.options[dataCombo.selectedIndex].getAttribute('requiere_centro_costo');
    }
    ...
    function obtenerMontoXPorcentaje(indice, fila) {
      ...
      if (porcentajeAcumulado > 100) {
        var nuevo_porcentaje = redondearXDecimales(100 - (porcentajeAcumulado - porcentaje), 6);
    ...
    function obtenerPorcentajeXMonto(indice, fila) {
      ...
      if (importeAcumulado > obtenerMontoTotalesXFila(fila)) {
        var nuevo_importe = redondearXDecimales(obtenerMontoTotalesXFila(fila) - importeAcumulado, 2);
        var porcentaje = (nuevo_importe / obtenerMontoTotalesXFila(fila)) * 100;
        document.getElementById("porcentaje_distribucion" + indice).value = redondearXDecimales(porcentaje, 6);
      ...
      document.getElementById("porcentaje_distribucion" + indice).value = redondearXDecimales(porcentaje, 4);
    ...
    function obtenerAcumuladoPorcentaje_MontoDistribucion(tipo) {
      ...
      $('#distribucion_body tr.fila_dato').each(function() {
        ...
        sumaPorcentaje = redondearXDecimales((m_porcentaje * 1) + sumaPorcentaje, 4);

        var m_monto = $(this).children('td.monto_td').children('input').val();
        m_monto = parseFloat(m_monto);
        sumaMontos = redondearXDecimales((m_monto * 1) + sumaMontos, 2);
      });
    ...
    function validarDistribucion(fila) {
      ...
      $.each(data, function(index, item) {
        if (isEmpty(item.centro_costo) && obtenerTipoOperacionValidarCentroCosto() == 1) {
        ...
        sumaPorcentaje = redondearXDecimales(item.porcentaje * 1 + sumaPorcentaje, 6);
        ...
        sumaMontos = redondearXDecimales(item.monto * 1 + sumaMontos, 2);
      });
    ...
    function guardarDistribucionContable() {
      ...
      $('#distribucion_body tr.fila_dato').each(function() {
        ...
        if (isEmpty(m_centro_costo) && obtenerTipoOperacionValidarCentroCosto() == 1) {
        ...
        sumaPorcentaje = redondearXDecimales((m_porcentaje * 1) + sumaPorcentaje, 6);
        ...
        sumaMontos = redondearXDecimales((m_monto * 1) + sumaMontos, 6);
      ...
      document.getElementById("lid_operacion_tipo" + dist_fila).value = $("#cboContOperacionTipo").val();
    ...
    function redondearXDecimales(numero, decimales) {
      decimales = (isEmpty(decimales) ? 2 : decimales * 1);
    ...
    function cargarDistribucion(fila) {
      ...
      if (!isEmpty(distribucion)) {
        ...
        $.each(data, function(index, item) {
          ...
          s_add += '<td class="centro_costo_td">' + centro_costo_t + '</td>'; //Centro Costo
          s_add += '<td class="porcentaje_td"><input type="number" value="' + (!isEmpty(item.porcentaje) ? (item.porcentaje * 1).toFixed(4) : 0) + '" min="0.1" max="100" id="porcentaje_distribucion[{0}]" name="porcentaje_distribucion[{0}]" onkeyup="obtenerMontoXPorcentaje(\'[{0}]\',\'' + fila + '\')"></td>';
          ...
          if (obtenerTipoOperacionValidarCentroCosto() == 0) {
            document.getElementById("centro_costo[" + identificador + "]").value = "";
    ...
    function agregarFilaDistribucion() {
      let contOperacionTipoId = document.getElementById("cboContOperacionTipo").value;
      if (isEmpty(contOperacionTipoId)) {
        alert('Primero seleccione la operacion contable relacionado a este documento');
        return;
      }
      ...
      var identificador = i;
      var s_add = '<tr class="fila_dato">';
      s_add += '<td class="indice_distribucion_td" width="5%">' + ($('#distribucion_body tr.fila_dato').length + 1) + '</td>';
      s_add += '<td class="cuenta_contable_td" width="30%">' + cuenta_contable_t + '</td>'; //Cuenta Contable
      s_add += '<td class="centro_costo_td"  width="30%">' + centro_costo_t + '</td>';
      s_add += '<td class="porcentaje_td" width="15%"><input type="number" value="0" min="0.1" max="100" id="porcentaje_distribucion[{0}]" name="porcentaje_distribucion[{0}]" onkeyup="obtenerMontoXPorcentaje(\'[{0}]\',\'' + dist_fila + '\')"  width="15%"></td>';
      s_add += '<td class="monto_td" width="15%"><input type="number" value="0" min="0.1" id="monto_distribucion[{0}]" name="monto_distribucion[{0}]" onkeyup="obtenerPorcentajeXMonto(\'[{0}]\',\'' + dist_fila + '\')"  width="15%"></td>';
      s_add += '<td  width="5%">' + dele + '</td>';
      s_add += '</tr>';
      var template = jQuery.validator.format(s_add);
      $tbl.after($(template(i++)));
...
<body>
  ...
  <h1>Revisar Liquidaci&oacute;n EAR <?php echo $zona_nom; ?></h1>

  <form id="liquidacion" action="ear_liq_act_vb_edit_p.php" method="post">
    <table>
      <tr>
      ...
      <tr>
        <td>Fecha de Liquidaci&oacute;n</td>
        <td>
          <?php
          ...
          <!--                <input type="text" value="<?php // echo $fechaLiqFormato;
                                        ...
                                        <span style="cursor: pointer" onclick="obtenerTipoCambioXFechaLiq()"><img src="img/reload.gif" border="0" title="Actualizar tipo de cambio" class="iconos"></span>                        -->
          <?php
          //            } else {
          echo $ear_liq_fec;
          //            }
      ...
      <tr>
        ...
        <td>
          <select id="periodoId" name="periodoId">
            <?php
            echo "<option value='-1'>Por documento</option>\n";
            $arrPer = obtenerPeriodoAbiertoSGI($id);

            if ($periodo_id == null) {
              $periodo_id = -1;
              //ASIGNA EL PERIODO DE LA FECHA DE LA LIQUIDACION
              //                $fechaArray = explode("-", $ear_liq_fec);
              //                $anio = $fechaArray[0];
              //                $mes = $fechaArray[1];
              //                $dia = $fechaArray[2];
              //
              //                foreach ($arrPer as $itemPer) {
              //                    if ($itemPer['anio']*1 == $anio*1 && $itemPer['mes']*1 == $mes*1) {
              //                        $periodo_id = $itemPer['id'];
              //                    }
              //                }
            }

            foreach ($arrPer as $v) {
              $selected = ($v['id'] * 1 == $periodo_id * 1) ? "selected" : "";
              echo "<option value=" . $v['id'] . " $selected>" . $v['anio'] . " | " . $v['mes'] . "</option>\n";
            }
            ?>
          </select>
        </td>
      ...
      <tr>
        <td></td>
        <td>* Si selecciona "Por documento", al dar visto bueno cada documento ser&aacute; registrado en el periodo seg&uacute;n su fecha de emisi&oacute;n.</td>
      </tr>
    ...
    <br>
    <div>Subtotal de los vi&aacute;ticos registrados en la solicitud y la liquidaci&oacute;n (topes) <img src="img/minus.png" id="sol_via_detalle_btn" title="Ocultar" class='iconos'></div>
    <table border="1" id="sol_via_detalle_tbl">
      ...
      <tr>
        <td>02</td>
        <td>Alimentaci&oacute;n / Pensi&oacute;n</td>
    ...
    <table border="1" id="doc_sust_detalle">
      <tbody id="bole_body">
        <tr>
          <td colspan="26">Boletos de Viaje / Pasajes A&eacute;reos</td>
        </tr>
        <tr>
          ...
          <td class="encabezado_h">Deducible</td>
          <td class="encabezado_h">Acciones</td>
        </tr>
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '01', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody>
        <tr>
          <!--<td colspan="21">Nueva fila Boletos de Viaje / Pasajes A&eacute;reos <img src="img/plus.png" id="bole_add" title="Agregar" class="iconos"></td>-->
        </tr>
      </tbody>
      ...
      <tbody id="alim_body">
        <tr>
          <td colspan="26">Alimentaci&oacute;n / Pensi&oacute;n</td>
        </tr>
        <tr>
          <td class="encabezado_h">Concepto</td>
          <td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
        ...
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '02', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody>
        <tr>
      ...
      <tbody id="hosp_body">
        ...
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '03', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody>
        <tr>
      ...
      <tbody id="movi_body">
        <tr>
          <td colspan="26">Movilidad / Combustible</td>
        </tr>
        <tr>
          <td class="encabezado_h">Concepto</td>
          <td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
        ...
        <?php if (!is_null($pla_id)) echo getFilaPlanillaMovilidad($pla_id, $mon_id, 2);
        else echo getFilaVaciaPlaMov(2); ?>
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '04', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody>
        <tr>
      ...
      <tbody id="gast_body" style="display: none;">
        <tr>
          <td colspan="26">Gastos de Representaci&oacute;n</td>
        </tr>
        <tr>
          <td class="encabezado_h">Concepto</td>
          <td class="encabezado_h" title="Tipo de documento">Tipo Doc</td>
        ...
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '05', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody style="display: none;">
        <tr>
          <!--    <td colspan="21">Nueva fila Gastos de Representaci&oacute;n <img src="img/plus.png" id="gast_add" title="Agregar" class="iconos"></td>-->
        </tr>
      </tbody>
      ...
      <tbody id="otro_body">
        ...
        <?php echo getFilasPrevias($arrLiqDet, $mon_id, '06', 3, 0, $arrDistribucionDetalle, 1); ?>
      </tbody>
      <tbody>
        <tr>
    ...
    <input type="hidden" value="<?php echo $lid_gti_def; ?>" name="lid_gti_def" id="lid_gti_def"></input>
    <input type="hidden" value='<?php echo $lid_dg_json_def; ?>' name="lid_dg_json_def" id="lid_dg_json_def"></input>
    <input type="submit" value="Actualizar Liquidacion" name="grabar" id="grabar" disabled></input>
    <!--<input type="submit" value="Aprobar Liquidacion" name="aprobar" id="aprobar" disabled>-->

    <p>Descripci&oacute;n de los botones:<br>
      <b>Actualizar</b> Actualizar los datos de la liquidaci&oacute;n.<br>
    </p>
  ...
  <div id="tipo_doc_template">
    <select class="tipo_doc" id="tipo_doc[{0}]" name="tipo_doc[{0}]">
      ...
      foreach ($arr as $v) {
        if ($v[11] == 1) {
          echo "<option value='$v[0]' rucreq='$v[2]' aplret='$v[3]' apldet='$v[4]' taxcode='$v[8]' apldistri='$v[12]'  sgidocumentoid='$v[13]'   " . ($v[0] == "2" ? " selected" : "") . ">$v[1]</option>\n";
        }
      }
      ?>
    </select>
  ...
  <div id="tipo_doc_nota_template">
    <select class="tipo_doc" id="tipo_doc[{0}]" name="tipo_doc[{0}]">
      ...
      foreach ($arr as $v) {
        if ($v[11] == 1) {
          echo "<option value='$v[0]' rucreq='$v[2]' aplret='$v[3]' apldet='$v[4]' taxcode='$v[8]' apldistri='$v[12]'" . ($v[0] == "2" ? " selected" : "") . ">$v[1]</option>\n";
        }
      }
      ?>
    </select>
  ...
  <div id="tipo_mon_template">
    <select class="tipo_mon" id="tipo_mon[{0}]" name="tipo_mon[{0}]" disabled>
      <?php
      $arr = getTipoMon();
      ...
      <div class="modal-body">
        <table border="1" id="table_distribucion" width=100%>
          <tbody id="distribucion_body">
            <tr>
              <td colspan="2">Distribuci&oacute;n Contable</td>
              <td colspan="4" id="tdContOperacionTipoId"></td>
            </tr>
            <tr>
              <td class="encabezado_h" width=5%>#</td>
              <td class="encabezado_h" width=30%>Cuenta Contable</td>
              <td class="encabezado_h" width=30%>Centro Costo</td>
          ...
          <tbody>
            <tr>
              <td colspan="6">Nueva fila <img src="img/plus.png" title="Agregar" class="iconos" onclick="agregarFilaDistribucion();" /></td>
            </tr>
          </tbody>
        </table>
      </div>
