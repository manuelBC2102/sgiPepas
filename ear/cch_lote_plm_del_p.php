...
// else {
	// if($arr[20]!=1) {
		// echo "<font color='red'><b>ERROR: Caja chica esta cerrada, no se puede quitar la planilla.</b></font><br>";
		// exit;
	// }
// }
//$ccl_id = $arr[0];
...
$stmt = $mysqli->prepare("INSERT INTO cajas_chicas_lote_act VALUES (?, 32, ?, ?, ?)") or die ($mysqli->error);
$stmt->bind_param('iiss', $ccl_id, $_SESSION['rec_usu_id'], $ahora, $pla_numero);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("UPDATE pla_mov SET est_id=4, ccl_id=null WHERE pla_id=?") or die ($mysqli->error);
$stmt->bind_param('i', $id);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("INSERT INTO movimientos VALUES (null, ?, ?, ?, ?, ?)") or die ($mysqli->error);
$desc = "Quitar Planilla Movilidad $pla_tipo (".$id.") hecho por ".$_SESSION['rec_usu_nombre'];
...
header("Location: cch_lote_plm_del.php");
exit;
?>
