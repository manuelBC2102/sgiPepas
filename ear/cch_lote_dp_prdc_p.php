...
// Validaciones
if ($est_id != 1) {
	echo "<font color='red'><b>ERROR: El documento pendiente seleccionado no esta en estado pendiente</b></font><br>";
	exit;
}
if (!is_null($cldp_prdc_fec)) {
	echo "<font color='red'><b>ERROR: El documento pendiente seleccionado ya inicio proceso de descuento</b></font><br>";
	exit;
}
...
$stmt = $mysqli->prepare("UPDATE cajas_chicas_lote_docp SET cldp_prdc_fec=?, cldp_prdc_usu=? WHERE cldp_id=?") or die ($mysqli->error);
$stmt->bind_param('sii',
	$ahora,
	$usu_id,
	$id);
...
$desc = "Inicio de proceso de descuento del documento pendiente vencido (".$id.") ".$dp_numero." hecha por ".$_SESSION['rec_usu_nombre'];
...
$subject = "Inicio de proceso de descuento del documento pendiente vencido $dp_numero de ".getNombreTrabajador(getCodigoGeneral(getUsuAd($colab_id)));
$body = "Se ha iniciado el proceso de descuento del documento pendiente vencido $dp_numero de ".getNombreTrabajador(getCodigoGeneral(getUsuAd($colab_id)))." por el monto de $cldp_monto $mon_nom ";
$body .= "de la caja chica $cch_nombre.";
$body .= "\n\nSe realizara el reembolso a la caja chica $cch_nombre por concepto de descuento.";
...
$body = "Se ha iniciado el proceso de descuento del documento pendiente vencido $dp_numero de ".getNombreTrabajador(getCodigoGeneral(getUsuAd($colab_id)))." por el monto de $cldp_monto $mon_nom ";
$body .= "de la caja chica $cch_nombre.";
$body .= "\n\nEl usuario de Compensaciones debera ingresar al modulo Administracion ";
$body .= "de la web intranet y realizar el descuento correspondiente del documento pendiente vencido.";
//enviarCorreo($to, $cc, $subject, $body);
$ventana   = "admin/cch_docp_vencidos.php";
...
$subject = "Inicio de proceso de descuento del documento pendiente vencido $dp_numero de ".getNombreTrabajador(getCodigoGeneral(getUsuAd($colab_id)));
$body = "Se ha iniciado el proceso de descuento del documento pendiente vencido $dp_numero de ".getNombreTrabajador(getCodigoGeneral(getUsuAd($colab_id)))." por el monto de $cldp_monto $mon_nom.";
$body .= "\n\nEl usuario de Tesoreria debera esperar el correo de confirmacion de descuento de Compensaciones y luego ingresar al modulo Administracion ";
...
//enviarCorreo($to, $cc, $subject, $body);
$ventana   = "admin/cch_docp_reem.php";
$aprobador = getUsuAd(getUsuTesoreria());
$detalle   = ConstructorMail::armarDetalle($cc, $subject, $body, $attachString, $attachFilename);
ConstructorMail::enviarCorreoInfoConLink($id, $aprobador, $ventana, $var_modulo_ident, $detalle);


header("Location: cch_docp_consulta.php?opc=0");
exit;
?>
