...
<head>
<!--<title>Mantenedor Viaticos - Administraci�n - Minapp</title>-->
<style type="text/css">
...
<script>
function editar(id) {
	location.href="mant_viaticos_edit.php?id="+id;
}

function borrar(id, desc, cod, mon) {
	var valor=confirm('Est\u00e1 seguro que desea eliminar el viatico '+desc+' ('+cod+') '+mon+'?');

	if(valor==true) { location.href="mant_viaticos_del_p.php?id="+id; }
}
</script>
<style>
.iconos {
...
<h1>Mantenedor de vi&aacute;ticos<a href="mant_viaticos_new.php"><img src="img/new_icon24.png" border="0" title="Registrar nuevo viatico"></a></h1>
<p><font color='red'><b>Nota: Seguir la misma estructura de los codigos al crear nuevos registros.<br></b></font></p>
...
foreach ($arr as $v) {
	...
	if (is_null($v[5])) {
		...
		if ($v[1] != "030199") {
			...
			if (substr($v[1], 0, 2) == "03") {
				echo "\t\t\t<img src='img/delete.png' border='0' title='Borrar' class='iconos' onclick='borrar(\"$v[0]\", \"$v[2]\", \"$v[1]\", \"$v[4]\");'>\n";
			}
		}
		echo "\t\t</td>\n";
	}
