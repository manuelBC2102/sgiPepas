...
list($vm_id, $vm_nombre) = getVehMarcasInfo($id);
...
$stmt = $mysqli->prepare("UPDATE veh_marcas SET vm_nombre=? WHERE vm_id=?") or die ($mysqli->error);
$stmt->bind_param('si',
	$nom,
	$id);
$stmt->execute() or die ($mysqli->error);
...
$desc = "Editar marca de vehiculo ($id), nombre $nom ($vm_nombre), hecha por ".$_SESSION['rec_usu_nombre'];
