...
$stmt = $mysqli->prepare("DELETE FROM ear_viaticos WHERE via_id=?") or die ($mysqli->error);
$stmt->bind_param('i', $id);
$stmt->execute() or die ("<b>ERROR (Consulte con TI)</b> ".$mysqli->error);

$stmt = $mysqli->prepare("INSERT INTO movimientos VALUES (null, ?, ?, ?, ?, ?)") or die ($mysqli->error);
$desc = "Viatico eliminado ($id), nombre $via_nom, moneda $mon_nom, tope $via_monto, hecha por ".$_SESSION['rec_usu_nombre'];
