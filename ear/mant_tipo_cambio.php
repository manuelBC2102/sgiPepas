...
<head>
<title>Mantenedor Tipo de Cambio - Administraci�n MinappES</title>
<style type="text/css">
...
<script>
function editar(id) {
	location.href="mant_tipo_cambio_edit.php?id="+id;
}

function borrar(id, desc, pre) {
	var valor=confirm('Est\u00e1 seguro que desea eliminar el tipo de cambio del dia '+desc+' ('+pre+') ?');

	if(valor==true) { location.href="mant_tipo_cambio_del_p.php?id="+id; }
}
</script>
<style>
.iconos {
...
<h1>Mantenedor Tipo de Cambio <a href="mant_tipo_cambio_new.php"><img src="img/new_icon24.png" border="0" title="Registrar nuevo tipo de cambio"></a></h1>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
	...
	<tbody>
<?php
$arr = getTipoCambioLista();

foreach ($arr as $v) {
	echo "\t<tr class='gradeA'>\n";
	echo "\t\t<td>".$v[0]."</td>\n";
	echo "\t\t<td><span title='".$v[1]."'>".$v[2]." <img src='".$v[3]."' class='iconos'></span></td>\n";
	echo "\t\t<td>".$v[4]."</td>\n";
	echo "\t\t<td align='right'>".$v[5]."</td>\n";
	
	echo "\t\t<td>\n";
	echo "\t\t\t<img src='img/edit.png' border='0' title='Editar' class='iconos' onclick='editar(\"$v[0]\");'>\n";
	echo "\t\t\t<img src='img/delete.png' border='0' title='Borrar' class='iconos' onclick='borrar(\"$v[0]\", \"$v[4]\", \"$v[5]\");'>\n";
	echo "\t\t</td>\n";
