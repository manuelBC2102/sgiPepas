...
if (isset($f_conc_l)) {
  foreach ($f_conc_l as $k => $v) {
    ...
    $lid_deducible = $f_doc_deducible[$k];
    $veh_id = null;
    $veh_km = null;
    $lid_glo2 = null;
    $lid_mon_otro = $f_montOtro_inp[$k];
    $lid_mon_icbp = $f_montIcbp_inp[$k];
    $lid_operacion_tipo = $f_lid_operacion_tipo[$k];
    ...
    if ($est_id == 6 && $doc_id == 10) {
      ...
      $stmt = $mysqli->prepare("INSERT INTO ear_liq_detalle (ear_id, conc_id, doc_id, ruc_nro, lid_fec, lid_ser, lid_nro, lid_glo, mon_id,
				...
				veh_id, veh_km, lid_glo2,lid_deducible, lid_mon_otro, documento_relacionado_id,lid_mon_icbp,operacion_tipo_id, monto_igv, orden_trabajo_id)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?, ?)") or die($mysqli->error);
      $stmt->bind_param(
        'iiisssssiidddiidissidiisidsdidi',
        $id,
        $conc_id,
        $doc_id,
        $ruc_nro,
    ...
    if ($est_id == 6) {
      $stmt = $mysqli->prepare("INSERT INTO ear_liq_detalle_hist (ear_id, hist_id, conc_id, doc_id, ruc_nro, lid_fec, lid_ser, lid_nro, lid_glo, mon_id,
				lid_afe, lid_mon_afe, lid_mon_naf, lid_tc, lid_retdet_apl, lid_retdet_tip, lid_retdet_mon, lid_gti, lid_dg_json, lid_cta_cont, lid_aprob, lid_emp_asume,
				veh_id, veh_km, lid_glo2, lid_mon_otro,documento_relacionado_id, orden_trabajo_id)
				VALUES (?, 6, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)") or die($mysqli->error);
      $stmt->bind_param(
        'iiisssssiidddiidissidiisdsi',
        $id,
        $conc_id,
        $doc_id,
        $ruc_nro,
...
//if ($f_periodoId*1 == -1){   $f_periodoId = null; }
$stmt = $mysqli->prepare("UPDATE ear_solicitudes SET periodo_id=? WHERE ear_id=?") or die($mysqli->error);
$stmt->bind_param('ii', $f_periodoId, $id);
$stmt->execute() or die($mysqli->error);

//REGISTRO DE LA SELECCION DE DUA
//if ($f_dua*1 == -1){
//    $f_dua = null;
//}
...
if ($est_id == 6) {
  ...
  $subject = "Liquidacion Actualizada de EAR $ear_numero de " . $ear_tra_nombres;
  $body = "Se ha actualizado la liquidacion del EAR $ear_numero de $ear_tra_nombres. por contabilidad.";
  ...
  $plantillaCuerpo = str_replace("[|asunto|]", 'Liquidaci&oacute;n Actualizada de EAR', $plantillaCuerpo);
...
header("Location: ear_liq_act_vb_edit_res.php?id=$id&o=$oper");
exit;
