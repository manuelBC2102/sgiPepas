...
$stmt = $mysqli->prepare("DELETE FROM tipo_cambio WHERE tc_id=?") or die ($mysqli->error);
$stmt->bind_param('i',
	$id);
$stmt->execute() or die ($mysqli->error);

$stmt = $mysqli->prepare("INSERT INTO movimientos VALUES (null, ?, ?, ?, ?, ?)") or die ($mysqli->error);
$desc = "Eliminar tipo de cambio ($id), fecha ($tc_fec), precio ($tc_precio), hecha por ".$_SESSION['rec_usu_nombre'];
