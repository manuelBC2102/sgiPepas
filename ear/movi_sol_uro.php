...
$arr = getLiqDesembolsadas($slave_usu_id);
$arr2 = getCajasChicasActivas();
if (count($arr)+count($arr2) == 0) {
	echo "<font color='red'><b>ERROR: No existen liquidaciones EAR o Cajas Chicas disponibles para agregar planilla de movilidad.</b></font><br>";
	exit;
}
...
$(document).ready(function()
{
	...
	$.fn.checktopes_redraw = function() {
		...
		$('.fecdoc_inp').each(function(){
			...
			if (monto <= 0) {
				html = '<div style="font-weight:bold;color:white;background-color:red; text-align:center; width:100px;">Inv�lido</div>';
			}
...
<table>
<tr><td align="right">Nombre colaborador:</td><td><?php echo $nombres; ?></td></tr>
...
<tr><td align="right">Tope m�ximo diario:</td><td><?php echo $tope_maximo." $mon_nom ($mon_simb) ($mon_iso) <img src='$mon_img' style='vertical-align:text-top'>"; ?></td></tr>
<tr>
...
?>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="radio" name="tipo_inp" value="2" <?php echo (count($arr2)>0?(count($arr)==0?'checked':''):'disabled'); ?>> Caja chica
		<select name="tipo2_sel" id="tipo2_sel"<?php echo (count($arr2)!=0?'':'disabled'); ?>>
<?php
foreach ($arr2 as $v) {
...
Comentarios del colaborador: (m�ximo 300 caracteres)<br>
