...
<head>
<title>Mantenedor Tipos de Documentos - Administraci�n MinappES</title>
<style type="text/css">
...
<script>
function editar(id) {
	location.href="mant_doc_tipos_edit.php?id="+id;
}

function borrar(id, desc, cod) {
	var valor=confirm('Est\u00e1 seguro que desea eliminar el documento '+desc+' ('+cod+')?');

	if(valor==true) { location.href="mant_doc_tipos_del_p.php?id="+id; }
}
</script>
<style>
.iconos {
...
<h1>Mantenedor Tipos de Documentos <a href="mant_doc_tipos_new.php"><img src="img/new_icon24.png" border="0" title="Registrar nuevo tipo de documento"></a></h1>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
	<thead>
		<tr>
			...
			<th>Abreviado</th>
			<th>Codigo</th>
			<th title='Requiere RUC'>Req RUC</th>
			<th title='Aplica Retenci�n'>Retenible</th>
			<th title='Aplica Detracci�n'>Detraible</th>
			<th>TAX Code</th>
			<th>Activo</th>
			<th>Acciones</th>
		</tr>
...
foreach ($arr as $v) {
	...
	echo "\t\t<td>".$v[1]."</td>\n";
	echo "\t\t<td>".$v[7]."</td>\n";
	echo "\t\t<td>".getSiNo($v[2])."</td>\n";
	echo "\t\t<td>".getSiNo($v[3])."</td>\n";
	echo "\t\t<td>".getSiNo($v[4])."</td>\n";
	echo "\t\t<td>".getTaxCodeDesc($v[8])."</td>\n";
	...
	if ($v[10]==1) {
		echo "\t\t\t<img src='img/delete.png' border='0' title='Borrar' class='iconos' onclick='borrar(\"$v[0]\", \"$v[1]\", \"$v[7]\");'>\n";
	}
	else {
		echo "\t\t\t<img src='img/transparent.gif' border='0' class='iconos'>\n";
	}
