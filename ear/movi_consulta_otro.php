...
// Valores cons_id: (ESTAN HABILITADOS 1 Y 3)
...
// 4: Consultar las Planillas de Movilidad de los usuarios asignados de alguien.
...
if ($cons_id < 1 || $cons_id > 4) {
	...
	$count = getPermisosAdministrativos($usu_id, 'JEFEOGERENTE');
	if ($count == 0) {
		$error_acceso = 1;
	}
}
else if ($cons_id == 3) {
	$count = getPermisosAdministrativos($usu_id, $pADMINIST);
	$count += getPermisosAdministrativos($usu_id, $pTI);
	if ($count == 0) {
		$error_acceso = 1;
	}
}
...
<head>
<!--<title>Reporte de Planillas de Movilidad para EAR  / Caja Chica- Administraci�n - Minapp</title>-->
	...
	$(document).ready(function() {
		var oTable = $('#example').dataTable( {
            ...
            $('#slave_usu_id').on('change', function(){
                ...
                location.href="movi_consulta_otro.php?cons_id=1&usuId="+usuId;
            });
...
<!--<h1>Reporte de Planillas de Movilidad para EAR / Caja Chica</h1>-->
<h1>Consulta de planillas de movilidad <span id="nombreUsuario">(<?php echo strtoupper($nombresUsuario); ?>)</span></h1>
...
<div class="full_width" style="margin-top: 1em; margin-bottom: 1em; padding-left: 1em; padding-right: 1em; width: auto;">
	...
	<tbody>
<?php
$arr = getPlanillasMovilidad($cons_id, $usu_id);
...
foreach ($arr as $v) {
	...
	if ($v['pla_tipo'] == 'CCH' && $v['est_id'] >= 4) {
		echo "<a href='cch_plm_pdf.php?id=".$v['pla_id']."'><img src='img/pdf.gif' border='0' title='Descargar PDF planilla CCH' class='iconos'></a>\n";
	}
	echo "</td>\n";
...
?>
	<!--<img src='img/pdf.gif' title='Descargar PDF planilla CCH'> Descargar PDF planilla CCH &nbsp;&nbsp;&nbsp;-->
</p>
