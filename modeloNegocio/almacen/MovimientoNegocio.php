...
class MovimientoNegocio extends ModeloNegocioBase
{
  ...
  public function obtenerConfiguracionInicial($opcionId, $empresaId, $usuarioId, $documentoId = null)
  {
    ...
    $respuesta->uo = null;
    if($respuesta->documento_tipo[0]["id"] == Configuraciones::GENERAR_COTIZACION || $respuesta->documento_tipo[0]["id"] == Configuraciones::GENERAR_COTIZACION_SERVICIO){
  ...
  public function obtenerDocumentoTipo($opcionId, $usuarioId = null)
  {
    ...
    if ($movimientoTipoId == Configuraciones::MOVIMIENTO_TIPO_SOLICITUD_REQUERIMIENTO) {
      ...
      $dataPerfil = PerfilNegocio::create()->obtenerPerfilXUsuarioId($usuarioId);
      foreach ($dataPerfil as $itemPerfil) {
        if ($itemPerfil['id'] == PerfilNegocio::PERFIL_ADMINISTRADOR_ID || $itemPerfil['id'] == PerfilNegocio::PERFIL_ADMINISTRADOR_TI_ID || $itemPerfil['id'] == PerfilNegocio::PERFIL_JEFE_LOGISTA || $itemPerfil['id'] == PerfilNegocio::PERFIL_LOGISTA || $itemPerfil['id'] == PerfilNegocio::PERFIL_TODAS_AREAS) {
          $mostrarTodasAreas = 1;
        }
      }
  ...
  public function validarGenerarDocumentoAdicional($opcionId, $usuarioId, $documentoTipoId, $camposDinamicos, $detalle, $documentoARelacionar, $valorCheck, $comentario = NULL, $checkIgv = 1, $monedaId = null, $accionEnvio, $tipoPago, $listaPagoProgramacion, $anticiposAAplicar = null, $periodoId = null, $percepcion = null, $origen_destino = null, $importeTotalInafectas = null, $datosExtras = null, $detalleDistribucion = null, $contOperacionTipoId = null, $distribucionObligatoria = null, $igv_porcentaje = null, $dataStockReservaOk = null, $dataPostorProveedor = null, $listaPagoProgramacionPostores = null)
  {
    ...
    if(!ObjectUtil::isEmpty($dataPostorProveedor)){
      foreach($dataPostorProveedor as $itemPostor){
        $respuestaDataPostorProveedor = Documento::create()->guardar_documento_detalle($respuesta->documentoId, $itemPostor['proveedor_id'], $itemPostor['monedaId'], $itemPostor['tipoCambio'], $itemPostor['igv'], $itemPostor['uoId'], $itemPostor['tiempoEntrega'], $itemPostor['tiempo'], $itemPostor['condicionPago'], $itemPostor['diasPago'],$itemPostor['sumilla'], $itemPostor['referencia'], $usuarioId);
  ...
  public function guardarXAccionEnvio($opcionId, $usuarioId, $documentoTipoId, $camposDinamicos, $detalle, $documentoARelacionar, $valorCheck, $comentario, $checkIgv, $monedaId, $accionEnvio, $tipoPago = null, $listaPagoProgramacion = null, $atiende = null, $periodoId = null, $percepcion = null, $datosExtras = null, $contOperacionTipoId = null, $igv_porcentaje = null, $dataStockReservaOk = null)
  {
    ...
    if (!ObjectUtil::isEmpty($listaPagoProgramacion) && ($documentoTipoId != Configuraciones::GENERAR_COTIZACION || $documentoTipoId != Configuraciones::GENERAR_COTIZACION_SERVICIO)) {
    ...
    if (!in_array($documentoTipoId * 1, $this->arrayDocumentoTipoSinDetalle) && $documentoTipoId != Configuraciones::ENTREGA) {
      ...
      if($documentoTipoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoId == Configuraciones::GENERAR_COTIZACION_SERVICIO){
        if(ObjectUtil::isEmpty($detalle)){
          throw new WarningException("Los precios de detalle están vacíos o son de valor 0");
        }
        foreach($detalle as $itemValidar){
          $todosVacios = true;
          foreach ($itemValidar['detalle'] as $item) {
              if ($item["columnaCodigo"] === "37" && (float)$item["valorDet"] > 0) {
                  $todosVacios = false;
                  break;
              }
          }
          if ($todosVacios) {
              throw new WarningException("Todos los precios de detalle están vacíos o son de valor 0");
          } 
        }
      }
      foreach ($detalle as $item) {
        ...
        //validar valores precios detalle 37
        if($documentoTipoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoId == Configuraciones::GENERAR_COTIZACION_SERVICIO){
          $alMenosUnoMayorACero = false;
          foreach ($item['detalle'] as $itemDetalle) {
              if ($itemDetalle["columnaCodigo"] === "37" && (float)$itemDetalle["valorDet"] > 0) {
        ...
        if ($documentoTipoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoId == Configuraciones::REQUERIMIENTO_AREA || $documentoTipoId == Configuraciones::ORDEN_COMPRA || $documentoTipoId == Configuraciones::COTIZACION_SERVICIO || $documentoTipoId == Configuraciones::ORDEN_SERVICIO || $documentoTipoId == Configuraciones::GENERAR_COTIZACION_SERVICIO) {
        ...
        if (!ObjectUtil::isEmpty($item["detalle"])) {
          foreach ($item["detalle"] as $valor) {
            if (!ObjectUtil::isEmpty($valor['valorDet'])) {
              ...
              if($documentoTipoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoId == Configuraciones::GENERAR_COTIZACION_SERVICIO || $documentoTipoId == Configuraciones::RECEPCION){
  ...
  public function obtenerDocumentosXCriterios($opcionId, $criterios, $elemntosFiltrados, $columns, $order, $start)
  {
    ...
    $estado_cotizacionFormateado = Util::convertirArrayXCadena($estado_cotizacion);
  ...
  function obtenerDocumentoRelacion($documentoTipoOrigenId, $documentoTipoDestinoId, $movimientoId, $documentoId, $opcionId, $documentoRelacionados)
  {
    ...
    if($documentoTipoDestinoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoDestinoId == Configuraciones::GENERAR_COTIZACION_SERVICIO){
      $filtrados = [];
      ...
      if ($documentoTipoId == Configuraciones::GENERAR_COTIZACION || $documentoTipoId == Configuraciones::GENERAR_COTIZACION_SERVICIO) {
        ...
        foreach($arraydetalleXpostor as $indexPostor => $itemPostor){
          ...
          $camposDinamicos [$cont + 1] = array(
            ...
            "codigo" => "",
            "valor" => $itemPostor["tiempoEntregaText"]);
          ...
          $camposDinamicos [$cont + 4] = array(
            "id" =>"",
            "tipo" => "2",
            "opcional" => "1",
            "descripcion" => "Condición de pago",
            "codigo" => "",
            "valor" => $itemPostor["condicionPagoText"]); 
          $camposDinamicos [$cont + 5] = array(
              "id" =>"",
              "tipo" => "2",
              "opcional" => "1",
              "descripcion" => "Dias de pago",
              "codigo" => "",
              "valor" => $itemPostor["diasPago"]);
          $camposDinamicos [$cont + 6] = array(
              "id" =>"",
              "tipo" => "2",
              "opcional" => "1",
              "descripcion" => "Referencia",
              "codigo" => "",
              "valor" => $itemPostor["referencia"]);                
  ...
  public function generarDocumentoPDFRequerimiento($documentoId, $comentario, $tipoSalidaPDF, $url, $data)
  {
    ...
    if ($urgencia == 1) {
      ...
      $pdf->MultiCell(50, 5, 'Fecha:', 1, 'L', 1, 0, 135, $espacio + 35, true, 0, false, true, 5, 'M');
      $pdf->SetFont('helvetica', '', 6);
      $fecha2 =  ObjectUtil::isEmpty($resultadoMatriz[2]['fecha_creacion']) ? "" : date_format((date_create($resultadoMatriz[2]['fecha_emision'])), 'd/m/Y');
      $pdf->MultiCell(40, 5, $fecha2, 1, 'L', 1, 0, 145, $espacio + 35, true, 0, false, true, 5, 'M');
    } else {
      $pdf->SetFont('helvetica', 'B', 6);
      $pdf->MultiCell(60, 5, 'Solicitado por', 1, 'C', 1, 0, 45, $espacio, true, 0, false, true, 5, 'M');
  ...
  public function guardarDocumentoCotizacion($camposDinamicosConsolidado,$usuarioId, $documentoTipoId, $detalle, $respuesta, $periodoId, $opcionId, $banderaCotizacon, $listaPagoProgramacion = null, $respuestaCotizacion = null, $arraydetalleXpostor = null, $datosExtras = null)
  {
    //generar cotizacion
    ...
    $filtradosTipo1 = array_values(array_filter($camposDinamicosConsolidado, function($item) {
      return $item['tipo'] === "1" && !empty($item['valor']);
    }));

    $filtradosTipo2 = array_values(array_filter($camposDinamicosConsolidado, function($item) {
      return $item['tipo'] === "2" && !empty($item['valor']);
    }));
    ...
    $filtradosTipo36 = array_values(array_filter($camposDinamicosConsolidado, function($item) {
      return $item['tipo'] === "36" && !empty($item['valor']);
    })); 
    ...
    foreach ($filtradosTipo23 as $itemTipo23) {
      ...
      foreach ($configuraciones as $item) {
        switch ($item['tipo']) {
          case 1:
            foreach($filtradosTipo1 as $itemFiltradosTipo1){
              if($item['descripcion'] == $itemFiltradosTipo1['descripcion']){
                $camposDinamicosCotizacion[] = array(
                  "id" => $item['id'],
                  "tipo" => $item['tipo'],
                  "opcional" => "0",
                  "descripcion" => $itemFiltradosTipo1['descripcion'],
                  "codigo" => $itemFiltradosTipo1['codigo'],
                  "valor" => $itemFiltradosTipo1['valor']
                );
              }
            }
            break;
          case 2:
            foreach($filtradosTipo2 as $itemFiltradosTipo2){
              if($item['descripcion'] == $itemFiltradosTipo2['descripcion']){
                $camposDinamicosCotizacion[] = array(
                  "id" => $item['id'],
                  "tipo" => $item['tipo'],
                  "opcional" => "0",
                  "descripcion" => $itemFiltradosTipo2['descripcion'],
                  "codigo" => $itemFiltradosTipo2['codigo'],
                  "valor" => $itemFiltradosTipo2['valor']
                );
              }
            }
            break;
          case 4:
            if($item['descripcion'] == "Tiempo de entrega"){
              $valor = null;
              if($documentoTipoId == Configuraciones::ORDEN_SERVICIO && $filtradosTipo4[0]['valor'] == 508){//Inmediato
                $valor = 511;
              }else if($documentoTipoId == Configuraciones::ORDEN_SERVICIO && $filtradosTipo4[0]['valor'] == 509){//Días
                $valor = 512;
              }else if($documentoTipoId == Configuraciones::ORDEN_SERVICIO && $filtradosTipo4[0]['valor'] == 510){//Contraentrega
                $valor = 513;
              }
            }else{
              $valor = $filtradosTipo4[0]['valor'];
            }
            $camposDinamicosCotizacion[] = array(
              "id" => $item['id'],
              "tipo" => $item['tipo'],
          ...
          case 23: //Proveedor u otros
            ...
            break; 
          case 36: //Detraccion tipo
            $camposDinamicosCotizacion[] = array(
              "id" => $item['id'],
              "tipo" => $item['tipo'],
              "opcional" => "0",
              "descripcion" => $filtradosTipo36[0]['descripcion'],
              "codigo" => $filtradosTipo36[0]['codigo'],
              "valor" => $filtradosTipo36[0]['valor']
            );
            break;             
  ...
  public function generarDocumentoPDFGenerarCotizacion($documentoId, $comentario, $tipoSalidaPDF, $url, $data){
    ...
    $tabla .= '<tr>'
                  ...
                  foreach($documento_detalle as $index => $item){
                    ...
                    $texMoneda = "S/";
                    if($item['moneda_id'] == "4"){
                      $valorTotal = $valorTotal * $item['tipo_cambio'];
                      $tipoCambio = "(T.C:" . number_format($item['tipo_cambio'],2).")";
                    }
                    $tabla .= '<th style="text-align:right" colspan="2" >'. $tipoCambio . " " .$texMoneda." ".number_format($valorTotal, 2).'</th>';
                  }
    $tabla .= '</tr>'            
          .'</tfoot>';
  ...
  public function exportarPdfCotizacion($grupoProductoId, $tipoRequerimiento, $urgencia, $usuarioId, $documentoId = null, $opcionId = null)
  {
    ...
    if(ObjectUtil::isEmpty($documentoId)){
      ...
      $dataMovimientoDocumento = MovimientoBien::create()->obtenerMovimientoIdXDocumentoId($documentoId);
      $documentoTipoId = DocumentoTipoNegocio::create()->obtenerDocumentoTipoXDocumentoId($documentoId)[0]['id'];
    ...
    $detalleRequerimientos = array_filter($detalleRequerimientos, function($item) {
      return $item['cantidad'] > $item['cantidad_atendida'];
    });
    $detalleRequerimientos = array_values($detalleRequerimientos);
    //$tipoSalidaPDF: F-> guarda local
    $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
    ...
    if (!ObjectUtil::isEmpty($detalleRequerimientos)) {
      foreach ($detalleRequerimientos as $index => $item) {
        ...
        $tabla = $tabla . '<tr>'
          ...
          . '<td style="text-align:center"  width="8%"></td>'
          . '<td style="text-align:center"  width="8%"></td>'
          . '<td style="text-align:center"  width="10%">' . number_format($cantidad, 2) . '</td>'
          . '<td style="text-align:center"  width="10%">' . $item['simbolo'] . '</td>';
        if($documentoTipoId == Configuraciones::SOLICITUD_REQUERIMIENTO){
          $dataComentario = $item['movimiento_bien_comentario'];
        }else{
  ...
  public function exportarExcelCotizacion($grupoProductoId, $tipoRequerimiento, $urgencia, $usuarioId, $documentoId = null, $opcionId = null)
  {
    ...
    foreach ($detalleRequerimientos as $index => $item) {
      ...
      if(!ObjectUtil::isEmpty($resDetalle)){
        foreach($resDetalle as $itemDetalle){
          $texto_con_espacios = preg_replace('/<(\/)?[a-zA-Z0-9]+[^>]*>/', '$0 ', $itemDetalle['comentario']);
          $texto_sin_etiquetas = strip_tags($texto_con_espacios);
          $texto_decodificado = str_replace('&nbsp;', '  ', $texto_sin_etiquetas);
          $dataComentario .= $texto_decodificado." (".number_format($itemDetalle['cantidad_requerimiento'], 2).")"."\n";        }
      }else{
        if($documentoTipoId == Configuraciones::SOLICITUD_REQUERIMIENTO){
          $texto_con_espacios = preg_replace('/<(\/)?[a-zA-Z0-9]+[^>]*>/', '$0 ', $item['movimiento_bien_comentario']);
          $texto_sin_etiquetas = strip_tags($texto_con_espacios);
          $texto_decodificado = str_replace('&nbsp;', '  ', $texto_sin_etiquetas);
          $dataComentario = utf8_decode($texto_decodificado);        }
      }
