...
(function($) {
            ...
            setValue : function(value) { 
                ...
                switch(options.Theme) {
                    ...
                    default:
                        ...
                        var dataToggle = $(this).data('dataToggle');
                        buttonClickEvent(dataToggle,sel);
                        break;
                    case 'Light':
                        var state = $(this).data('state');
                        var selValue = !value ? options.OnValue : options.OffValue;
                        lightClickEvent($(this), id, !value, selValue);
                        break;
                    case 'Swipe':
                        var state = $(this).data('state');
                        var selValue = !value ? options.OnValue : options.OffValue;
                        swipeClickEvent(instance, id, !value, value)
                        break;
                    case 'iOS':
                        var state = $(this).data('state');
                        var selValue = !value ? options.OnValue : options.OffValue;
                        iosClickEvent($(this), id, !value, selValue);
                        break;
                    case 'Android':
                        var state = $(this).data('state');
                        var selValue = !value ? options.OnValue : options.OffValue;
                        androidClickEvent($(this), id, !value, selValue);
                        break;
                }
            },
            getValue : function( ) { 
            ...
            switch(settings.Theme) {
                ...
                default:
                    ...
                    $('#bsh-' + id + ' button').on('click', function() {
                        var sel = $(this).data('title');
                        $(this).data('dataToggle',dataToggle);
    ...
    $('#swFiltroPersonalizado').btnSwitch({
        Theme:'Android',
        OnCallback: function(){
            alert("switch Seleccionado");
            console.log($("#swFiltroPersonalizado").btnSwitch("getValue"));
        },
        OffCallback: function(){
            alert("switch Deseleccionado");
    ...
    setTimeout(function(){
        $("#swFiltroPersonalizado").btnSwitch("setValue",true);
    },2000);
