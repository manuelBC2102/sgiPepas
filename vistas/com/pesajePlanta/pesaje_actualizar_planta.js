    ...
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'validarSimilitud':
                ...
                loaderClose();
                break;
            case 'obtenerDataActaSolicitud':
                loaderClose();
                listarDataSolicitudes2(response.data);
                $('.modal-backdrop').hide();
                
                break;
            case 'guardarPesajeSolicitudRetiro':
                    mostrarOk("Pesajes solicitud retiro actualizada");
                    loaderClose();
                    habilitarBoton();
    ...
    } else
    {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerDataActaSolicitud':
                ...
                habilitarBoton();
                $('.modal-backdrop').hide();
                mostrarMensajeError("No se pudo obtener datos para la solicitud");
                
                break;
            case 'guardarPesajeSolicitudRetiro':
                mostrarMensajeError("No se pudo actualizar los pesajes  de la solicitud retiro");
                    loaderClose();
                    habilitarBoton();
                    break;
        }
...
function setearInputActa() {
    loaderShow();
    ax.setAccion("obtenerDataActaSolicitud");
...
function listarDataSolicitudes2(data) {
    ...
    var cabeza = '<div class="table-responsive">' + // Agregar clase table-responsive aquí
        ...
        "<th style='text-align:center;'>Guia Remisión Remitente</th>" +
        "<th style='text-align:center;'>Guia Remisión Transportista</th>" +
    ...
    $.each(data, function (index, item) {
        cuerpo = "<tr>" +
            ...
            "<td style='text-align:center;'>" + (item.archivo_factura ? "<a href='" + url + item.archivo_factura + "' class='btn btn-primary' style='color:white; background-color:#4686bd;' target='_blank'>Ver archivo</a>" : "<span style='color:white; background-color:#d03f3f;' class='badge badge-warning'>Archivo está por subir</span>") + "</td>" +
            "<td style='text-align:center;'>" + (item.archivo_guia ? "<a href='" + url + item.archivo_guia + "' class='btn btn-primary' style='color:white; background-color:#4686bd;' target='_blank'>Ver archivo</a>" : "<span style='color:white; background-color:#d03f3f;' class='badge badge-warning'>Archivo está por subir</span>") + "</td>" +
            "<td style='text-align:center;'>" + (item.archivo_guia_transportista ? "<a href='" + url + item.archivo_guia_transportista + "' class='btn btn-primary' style='color:white; background-color:#4686bd;' target='_blank'>Ver archivo</a>" : "<span style='color:white; background-color:#d03f3f;' class='badge badge-warning'>Archivo está por subir</span>") + "</td>" +
