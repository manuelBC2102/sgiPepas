...
function onresponseConfiguraciones(data)
{
        ...
        html='<div class="input-group-btn dropdown">'+
                '<button type="button" class="btn btn-info"  onclick="cargarFormularioPersona()"><i class=" fa fa-plus-square-o"></i>&nbsp; Nueva Valorizacion </button>'+
...
{    commonVars.personaId = 0;
    cargarDiv('#window', 'vistas/com/valorizacion/valorizacion_form.php', "Nueva ");
}
...
//    var clasePersona = $("#cboClasePersonaBusqueda").val();
    ax.setAccion("getDataGridValorizacion");
     if(id==null){
        ...
        "columns": [
            ...
            
            {
                ...
                "render": function(data, type, row) {
                    if (data==null || data=='En proceso.' ) 
                        {
                            return '<span style="color: greem;">COMPLETADO</span>';
                        }
                    if (type === 'display') {
...
//            },
{
    ...
    render: function (data, type, row) {
        if (type === 'display') {
            return '<a onclick="archivoGenerar(' + row.id + ', \'' + row.documento_efact + '\')"><b><i class="fa fa-file-archive-o" style="color:#cb2a2a;"></i></b></a>\n' +
            '<a onclick="archivoGenerar(' + row.id + ', \'' + row.documento_transportista + '\')"><b><i class="fa fa-file-archive-o" style="color:#18d64e;"></i></b></a>\n' +
                   '<a onclick="archivoGenerar(' + row.id + ', \'' + row.documento_carguio + '\')"><b><i class="fa fa-file-archive-o" style="color:#blue;"></i></b></a>';
        }
        return data;
    },
    orderable: true,
