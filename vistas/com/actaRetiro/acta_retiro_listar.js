...
function registrarPesajeFinal(id)
{
    commonVars.personaId=id;
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/actaRetiro/acta_retiro_form.php?id=' + id);
}
...
function onresponseConfiguraciones(data)
{
        ...
        html='<div class="input-group-btn dropdown">'+
                '<button type="button" class="btn btn-info"  onclick="cargarFormularioPersona()"><i class=" fa fa-plus-square-o"></i>&nbsp; Nueva Acta </button>'+
...
{    commonVars.personaId = 0;
    cargarDiv('#window', 'vistas/com/actaRetiro/acta_retiro_inicial_form.php', "Nueva ");
}
...
//    var clasePersona = $("#cboClasePersonaBusqueda").val();
    ax.setAccion("getDataGridActaRetiro");
     if(id==null){
        ...
        "columns": [
            ...
            {"data": "usuario"},
            {
                ...
                "render": function(data, type, row) {
                    if (type === 'display') {
                        if(row.pesaje==null){
                            return '<span style="color: red;">PENDIENTE PESAJE FINAL</span>';
...

{
    ...
    render: function (data, type, row) {
        ...
        if(row.pesaje==null){
            return '<a onclick="registrarPesajeFinal(' + row.id + ')"><b><i class="fa fa-pencil-square-o" style="color:#18d64e;"></i></b></a>\n' +
            '<a onclick="confirmarDeleteSolicitud(' + row.id + ')"><b><i class="fa fa-trash-o" style="color:#cb2a2a;"></i></b></a>';
                  
        }
         else{
            return '<a onclick="imprimirDocumentoTicket(' + row.id + ')"><b><i class="fa fa-file" style="color:#088a08;"></i></b></a>\n' +
            '<a onclick="confirmarDeleteSolicitud(' + row.id + ')"><b><i class="fa fa-trash-o" style="color:#cb2a2a;"></i></b></a>\n' +
            '<a onclick="archivoGenerar(' + row.id + ', \'' + row.guia_efact_transportista + '\')"><b><i class="fa fa-file-archive-o" style="color:#18d64e;"></i></b></a>\n' +
            '<a onclick="archivoGenerar(' + row.id + ', \'' + row.guia_efact + '\')"><b><i class="fa fa-file-archive-o" style="color:#33afff;"></i></b></a>';
