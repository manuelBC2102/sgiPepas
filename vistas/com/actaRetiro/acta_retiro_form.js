...
$(document).ready(function () {
    ...
    ax.setAccion("obtenerActaRetiroXId");
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/actaRetiro/acta_retiro_listar.php');
}
...
if (resultado) {
    ...
    loaderClose();
        ...
        cargarDiv('#window', 'vistas/com/actaRetiro/acta_retiro_listar.php', 'Listar Solicitudes Retiro');
    }
}
...
;
    ...
    $("#txtPesaje").val(data[0].pesaje_inicial);
    $("#txtFechaInicial").val(data[0].fecha_inicial);
    listarDataSolicitudes(data);
    //tablas sunat    
    select2.asignarValor('cboCarreta', data[0].carreta_id);
...
function guardarSolicitud() {
    ...
    // if(pesaje!=''){
     var fechaInicio = trim(document.getElementById('txtFechaInicial').value);
     var fechaFinal = document.getElementById('fechaPesajeFinal').querySelector('span').textContent; 
    // else {
        var fechaHora = new Date(); 
    //    var fechaInicio= fechaHora.toLocaleString();
    //   var fechaFinal=fechaHora.toLocaleString();
    
    // var fechaFinal = trim(document.getElementById('fechaPesajeFinal').value);
    var valorCarreta = document.querySelector('input[name="carreta"]:checked').value;
    if(valorCarreta=='si'){
...
function toggleCarreta(hasCarreta) {
    ...
    if (hasCarreta==false) {
        carretaSelectContainer.style.display = 'none';
         // Cargar las placas disponibles si se tiene carreta
    } else {
...
function listarDataSolicitudes(data) {
    ...
    var cabeza = '<table id="datatable" class="table table-striped table-bordered"><thead>' +
        "<tr>" +
        "<th style='text-align:center;'><input checked disabled  type='checkbox' id='select_all' /></th>" +
    ...
    $.each(data, function (index, item) {
        cuerpo = "<tr>" +
            "<td style='text-align:center;'><input type='checkbox' checked disabled  class='select_item' data-id='" + item.solicitud_id + "' /></td>" +
            ...
            "<td style='text-align:left;'>" + item.planta_nombre + "</td>" +
