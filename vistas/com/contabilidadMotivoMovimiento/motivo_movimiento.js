$(document).ready(function () {
    loaderClose();
    ax.setSuccess("exitoMotivosMovimiento")
    listarMotivosMovimiento();
});

function listarMotivosMovimiento()
{
    ax.setAccion("listarMotivosMovimiento");
    ax.addParamTmp("empresaID", commonVars.empresa);
    ax.consumir();
}

function exitoMotivosMovimiento(response)
{
    if (response.status === 'ok')
    {
        switch (response[PARAM_ACCION_NAME])
        {
            case 'listarMotivosMovimiento':
                onResponseListarMotivosMovimiento(response.data);
                onResponseVacio();
                loaderClose();
                break;
            case 'cambiarEstado':
            ...
            case 'eliminar':
                ...
                if (error > 0) {
                    swal("Eliminado!", "El Motivo de Movimiento: " + response.data['0'].descripcion + ".", "success");
                    listarMotivosMovimiento();
                } else {
                    swal("Cancelado", "El Motivo de Movimiento: " + response.data['0'].descripcion + ". " + response.data['0'].vout_mensaje, "error");
                }
...
function onResponseListarMotivosMovimiento(data)
{
    $("#dataList").empty();
    var cuerpo_total = "";
    var cuerpo = "";
    ...
    if (!isEmpty(data)) {
        $.each(data, function (index, item) {
            ...
            cuerpo_acc = cuerpo_acc
                    ...
                    + "<a href='#' onclick='confirmarEliminar(" + item.id + ",\"" + item.motivo_descripcion + "\")'><i class='fa fa-trash-o' style='color:#cb2a2a;'></i></a>&nbsp;\n";

            cuerpo = "<tr>"
                    + "<td style='text-align:left;'>" + item.motivo_codigo + "</td>"
                    + "<td style='text-align:left;'>" + item.motivo_descripcion + "</td>"
...
function nuevo() {
    var titulo = "Nuevo";
    var url = URL_BASE + "vistas/com/contabilidadMotivoMovimiento/motivo_movimiento_form.php?winTitulo=" + titulo;
    cargarDiv("#window", url);
}

function editar(id) {
    var titulo = "Editar";
    var url = URL_BASE + "vistas/com/contabilidadMotivoMovimiento/motivo_movimiento_form.php?winTitulo=" + titulo + "&id=" + id;
    cargarDiv("#window", url);
}
...
function confirmarEliminar(id, nom) {
    ...
    swal({
        title: "Estás seguro?",
        text: "Eliminarás el Motivo de Movimiento: " + nom + "!",
        type: "warning",
