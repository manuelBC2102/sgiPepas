        ...
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerPaqueteXAlmacenId':
                for (let index = 0; index < nroFilasInicial; index++) {
                    cargarBienDetalleCombo(response.data, index);
                }
                break;
            ...
            case 'obtenerDetalleRequerimiento':
                onResponseObtenerDetalleRequerimiento(response.data);
                break;
        }
    } else {
        switch (response[PARAM_ACCION_NAME]) {
...
function agregarBienDetalleTabla(i) {
    var $html = "<div class=\"input-group col-lg-12 col-md-12 col-sm-12 col-xs-12\">" +
        ...
        "<input name=\"cboBien_" + i + "\" id=\"cboBien_" + i + "\" class=\"select2\" >" +
    ...
    if (!isEmpty(data)) {
        ...
        $("#cboBien_" + indice).select2({
            ...
            query: function (q) {
                ...
                results = [];
                if (q.term && q.term !== "") {
                    results = data.filter(itemProducto => itemProducto.text.toUpperCase().indexOf(q.term.toUpperCase()) >= 0);
                } else if (q.term === "") {
                    results = this.data;
                }
                q.callback({
...
function cargarPantallaListaDespacho() {
    cargarDiv("#window", "vistas/com/almacenes/entrega_listar.php?documento_tipo=289");
}
...
function onResponseObtenerAreRequerimiento(data) {
    ...
    select2.cargar("cbo_" + dtdrequerimientoId, data.requerimiento, "id", "serie_numero");
    loaderClose();
}
...
function onResponseObtenerDetalleRequerimiento(data) {
    $.each(data, function (index, item) {
