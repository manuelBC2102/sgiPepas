...
function llenarTablaDetalle(data) {
    ...
    //LLENAR COMBOS
    for (var i = 0; i < nroFilas; i++) {
        cargarOrganizadorDetalleCombo(data.organizadores, i);
    ...
    if (existeColumnaCodigo(33)) {
        $("#cboCompra_" + indice).select2({
            ...
            if (e.val == "1") {
                ...
                var bienId = select2.obtenerValor('cboBien_' + indice);
                dataStockOk = dataStockOk.filter(objeto => objeto.bien_id != bienId);
                $("#trDetalle_" + indice).css('background-color', '');
            }
            actualizarTotalesGenerales(indice);
        });
        ...
        $.each(data, function (index, item) {
            ...
            switch (parseInt(item.tipo)) {
                ...
                case 5:
                    ...
                    var filtro = item.data.filter(function (obj) {
                        return !isEmpty(obj.area_id);
                    });
                    if (!isEmpty(item.data)) {
                        $.each(filtro, function (indexPersona, itemPersona) {
            ...
            switch (parseInt(item.tipo)) {
                ...
                case 48:
                    $("#cbo_" + item.id).select2({
                        ...
                        obtenerDetalleRequerimiento(e.val);
    ...
    if (!isEmpty(dataColumna)) {
        $.each(dataColumna, function (index, item) {
            ...
            switch (parseInt(item.codigo)) {
                ...
                case 12:// CANTIDAD
                    $('#txtCantidad_' + indexDetalle).val(devolverDosDecimales(valoresFormularioDetalle.cantidad_entrega));
    ...
    $.each(camposDinamicos, function (index, item) {
        ...
        switch (item.tipo) {
            ...
            case 42: // Tipo de requerimiento
                ...
                break;
            case 53:
                camposDinamicos[index]["valor"] = select2.obtenerValor('cboOrganizador');
                break;
        }
    });
    return isOk;
...
function validarDetalleFormularioLlenos() {
    ...
    $.each(detalle, function (i, item) {
        ...
        if (existeColumnaCodigo(33)) {
            ...
            if (item.esCompra == 1) {
                var dataStockReservaBien = dataStockOk.find(itemReversa => itemReversa.bien_id == item.bienId);
                if (!isEmpty(dataStockReservaBien)) {
                    // if (cantidad_ == cantidadAceptada_) {
                    //     dataStockOk = dataStockOk.filter(objeto => objeto.bien_id != item.bienId);
                    //     valido = false;
                    //     return false;
...
function verificarStockBien(indice) {
    ...
    if (bienId == null) {
        ...
        obtenerStockPorBien(bienId, indice, unidadMedidaId);
    }
}
function obtenerStockPorBien(bienId, indice, unidadMedidaId) {
...
var organizadoresDetalle = [];
function onResponseObtenerDocumentoRelacion(data) {
    organizadoresDetalle = data.organizadoresDetalle;
    ...
    onResponseCargarOrganizadoresDetalle(data.organizadoresDetalle);
}
        ...
        $.each(data, function (index, item) {
            cargarDataTableDocumentoACopiar(
                cargarFormularioDetalleACopiar(item.organizador_id, item.bien_id, item.cantidad,
                    ...
                    item.movimiento_bien_ids, item.precio_postor1, item.precio_postor2, item.precio_postor3, item.postor_ganador_id, item.cantidad_atendida, item.cantidad_entrega
                )
            );
...
function cargarFormularioDetalleACopiar(organizadorId, bienId, cantidad, unidadMedidaId, precio,
    ...
    ageniaId, agenciaDescripcion, agrupadorBien, agrupadorBienDescripcion, ticket, CeCoId, movimiento_bien_ids, precio_postor1, precio_postor2, precio_postor3, postor_ganador_id, cantidad_atendida, cantidad_entrega
    ...
    objDetalle.cantidad_entrega = parseFloat(cantidad_entrega);
    return objDetalle;
}
...
function dibujarBotonesDeEnvio(data) {
    ...
    var html = '<a href="#" class="btn btn-danger" onclick="cargarPantallaListarEntrega()"><i class="fa fa-close"></i> Cancelar</a>';
    var accion = '';
    var estilo = '';
...
function onResponseObtenerStockPorBienReserva(dataStock, indice) {
    ...
    var tituloModal = '<strong>Strock</strong><br><strong>' + select2.obtenerText("cboBien_" + indice) + '</strong>';
    $('.modal-title').empty();
    $('.modal-title').append(tituloModal);
...
function onResponseObtenerAreRequerimiento(data) {
    ...
    select2.cargar("cbo_" + dtdareaId, data.area, "id", "descripcion");
    select2.asignarValor('cbo_' + dtdareaId, data.area[0].id);

    $("#cbo_" + dtdrequerimientoId).empty();

    if (!isEmpty(data.requerimiento)) {
        var html = '<option value="0" selected>Seleccionar</option>';
        $.each(data.requerimiento, function (index, item) {
            html += '<option value="' + item.id + '">' + item.serie_numero + '</option>';
        });
        $("#cbo_" + dtdrequerimientoId).append(html);
    }
    select2.asignarValor('cbo_' + dtdrequerimientoId, 0);
    loaderClose();
}

function obtenerDetalleRequerimiento(id) {
    loaderShow();
...
function onResponseCargarOrganizadoresDetalle(data) {
    organizadoresDetalle = data;
    var indexTemporal = -1;
    $.each(detalle, function (i, item) {
        cargarOrganizadorDetalleCombo(data, item.index);
    });
}