var estadoTolltipPagar = 0;
var banderaBuscarPagar = 0;
...
//    iniciarDataPicker();
    ...
    ax.setSuccess("onResponseReporteCuentasPorPagarGeneral");
    obtenerConfiguracionesInicialesReporteDeudaGeneral();
});

function onResponseReporteCuentasPorPagarGeneral(response) {
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerConfiguracionesInicialesPagarGeneral':
                onResponseObtenerConfiguracionesInicialesReporteDeudaGeneral(response.data);
                
                break;
            case 'obtenerDetallePago':
                onResponseDetallePagoGeneral(response.data);
                break;
        }
    } else {
        switch (response[PARAM_ACCION_NAME]) {
...
{
    ax.setAccion("obtenerConfiguracionesInicialesPagarGeneral");
...
var valoresBusquedaReportePagoGeneral = [{persona: "", fecha: "", bandera: "0"}];//bandera 0 es balance
...
function cargarDatosBusqueda()
{
    ...
    valoresBusquedaReportePagoGeneral[0].mostrar = mostrar;
    
    valoresBusquedaReportePagoGeneral[0].persona = PersonaId;
    valoresBusquedaReportePagoGeneral[0].empresa = empresaId;
    valoresBusquedaReportePagoGeneral[0].fecha = fecha;
//    valoresBusquedaReportePagoGeneral[0].fechaVencimiento = objetoFecha(fechaVencimientoInicio, fecha);
    getDataTable();
}
...
function obtenerDatosBusqueda()
{
    var cadena = "";
    cargarDatosBusqueda();
    if (valoresBusquedaReportePagoGeneral[0].persona != -1)
    {
    ...
    if (!isEmpty(valoresBusquedaReportePagoGeneral[0].fecha))
    {
        cadena += StringNegrita("Fecha: ");
        cadena += valoresBusquedaReportePagoGeneral[0].fecha;
        cadena += "<br>";
    }
    return cadena;
}
...
function getDataTable() {
    color = '';
    ax.setAccion("obtenerDataPagarGeneral");
    
    ax.addParamTmp("criterios", valoresBusquedaReportePagoGeneral);
...
//                        return '<a href="#" onClick = "agregarDocumentoPago(' + row.semaforo + ');" name = "btnpd_' + row.documento_id + '" id="btnpd_' + row.documento_id + '"><b><i class="fa fa-flag" style = "color:' + color + ';"></i><b></a>&nbsp;\n';
                        return "<a href='#' onclick='visualizarPagoGeneral(" + row.documento_id + ")' title='Visualizar'><b><i class='fa fa-eye' style='color:#1ca8dd;'></i><b></a>&nbsp;\n";
                    }
                    return data;
                },
//                "orderable": true,
...
{
    if (banderaBuscarPagar == 1)

    {
        if (estadoTolltipPagar == 1)
        {
    ...
    estadoTolltipPagar = (estadoTolltipPagar == 0) ? 1 : 0;
}
function visualizarPagoGeneral(id)
{
    loaderShow();
...
function onResponseDetallePagoGeneral(data)
{
    if (!isEmptyData(data))
    {
        $('[data-toggle="popover"]').popover('hide');
//        var stringTituloStock = '<strong> ' + data[0]['organizador_descripcion'] + ' - ' + data[0]['bien_descripcion'] + '</strong>';
        ...
        $('#datatableDetallePagoGeneral').dataTable({
            order: [[0, "desc"]],
            "ordering": false,
            "data": data,
            "columns": [
...
//            }
        ...
        $("#modal_detalle_pagos_general").modal("show");

    }
    else
    {
        var table = $('#datatableDetallePagoGeneral').DataTable();
        table.clear().draw();
