...
function agregarCantidadDetalleTabla(i) {
  ...
  if(doc_TipoId == SOLICITUD_REQUERIMIENTO || doc_TipoId == GENERAR_COTIZACION){
    disabled = "";
  }
  var $html =
    '<div class="input-group col-lg-12 col-md-12 col-sm-12 col-xs-12">' +
    ...
    ");hallarStockSaldo(" +
    i +
    ');hallarSubTotalPostorDetalleCantidad(' + i +');"/ '+ disabled +'></div><input type=\'hidden\' id=\'txtmovimiento_bien_ids_' + i + '\' name=\'txtmovimiento_bien_ids_' + i + '\' /><input type=\'hidden\' id=\'txtCantidadPorAtender_' + i + '\' name=\'txtCantidadPorAtender_' + i + '\' />';
...
function cargarPantallaListarCompra() {
  if (isEmpty(opcionAnterior)) {
    cargarDiv(
      "#window",
      "vistas/com/compraServicio/compra_listar.php?tipoInterfaz=" + tipoInterfaz
    );
  } else {
    cargarDiv(
      "#window",
    ...
    $.each(dataColumna, function (index, item) {
      ...
      switch (parseInt(item.codigo)) {
        ...
        default:
          ...
          if (doc_TipoId == GENERAR_COTIZACION && !isEmpty(movimientoBienDetalle)) {
            $.each(movimientoBienDetalle, function (indexBD, itemBD) {
...
function dibujarBotonesDeEnvio(data) {
  var html =
    '<a href="#" class="btn btn-danger" onclick="cargarPantallaListarCompra()"><i class="fa fa-close"></i> Cancelar</a>';
  var accion = "";
  var estilo = "";
