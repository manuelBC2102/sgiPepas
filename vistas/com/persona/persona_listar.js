...
//            $('#listaPersonaTipo').append('<li><a data-toggle="modal" data-target="#accordion-modal" onclick="cargarFormularioPersona(' + value.id + ',\'' + value.descripcion + '\',\'' + value.ruta + '\')">' + value.descripcion + '</a></li>');
        ...
        html='<div class="input-group-btn dropdown">'+
                '<button type="button" class="btn btn-info"  onclick="cargarFormularioPersona(' + perJuridica.id + ',\'' + perJuridica.descripcion + '\',\'' + perJuridica.ruta + '\')"><i class=" fa fa-plus-square-o"></i>&nbsp; Nueva ' + perJuridica.descripcion + '</button>'+
                ...
                '<ul class="dropdown-menu" role="menu">'+
                    '<li><a href="#" onclick="cargarFormularioPersona(' + perNatural.id + ',\'' + perNatural.descripcion + '\',\'' + perNatural.ruta + '\')"> Nueva ' + perNatural.descripcion + '</a></li>'+
                '</ul>'+
            '</div>';
...
//            },
            {data: "id",
                render: function (data, type, row) {
                    if (type === 'display') {
                        var tipo_clase = row.persona_clase_descripcion;
                        var checkbox = '<input type="checkbox" class="select-persona" data-id="' + row.id + '">';
                        if (tipo_clase.includes('REINFO')) { // Verificar si 'Reinfo' está presente en las clases
                            return '<a onclick="editarPersona(' + row.id + ',' + row.persona_tipo_id + ')"><b><i class="fa fa-edit" style="color:#E8BA2F;"></i></b></a>' +
                                ...
                                '<a onclick="agregarArchivos(' + row.id + ',' + row.persona_tipo_id + ')"><b><i class="fa fa-upload"></i></b></a>';
                        } else {
                            return '<a onclick="editarPersona(' + row.id + ',' + row.persona_tipo_id + ')"><b><i class="fa fa-edit" style="color:#E8BA2F;"></i></b></a>' +
...
function agregarArchivos(id, tipo) {
    // loaderShow(null);
    commonVars.personaId = id;
    commonVars.personaTipoId = tipo;
    cargarDiv("#window", "vistas/com/persona/persona_subir_archivo.php", "Subir " + obtenerTitulo());
    console.log(id,tipo);
}
