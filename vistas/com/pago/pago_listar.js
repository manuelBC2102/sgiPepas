...
//                break;
            ...
            case 'visualizarPago':
                ...
                loaderClose();
                break;
            case 'buscarCriteriosBusquedaDocumentoPagoListarPago':
                onResponseBuscarCriteriosBusquedaDocumentoPagoListarPago(response.data);
                loaderClose();
                break;
            case 'imprimir':
                loaderClose();
...
function getPagoDataTable() {
    ax.setAccion("obtenerDocumentosPagoListarConDocumentoPago");
    ax.addParamTmp("empresa_id", commonVars.empresa);
    ...
    $('#datatableListaPagos').dataTable({
        ...
        "columns": [
            ...
            {"data": "persona_nombre_completo", "width": "300px"},
//            {"data": "serie"},
            {"data": "numero"},
//            {"data": "fecha_vencimiento"},
            {"data": "monto", "class": "alignRight"},
            {"data": "total", "class": "alignRight"},
            {"data": "moneda_descripcion", "class": "alignLeft"},
...
function enviarCorreoDocumentoPago() {
    ...
    if (!isEmpty(correo) && valid) {
        ...
        ax.addParamTmp("tipoCobroPago", 2);//1->cobro, 2 -> pago
        ax.consumir();
    }else
    {
...
//    loaderShow();
    //buscarDocumentoPago();
    ax.setAccion("buscarCriteriosBusquedaDocumentoPagoListarPago");
    ax.addParamTmp("busqueda", $('#txtBuscar').val());
    ax.addParamTmp("empresa_id", commonVars.empresa);
    ax.consumir();
}

function onResponseBuscarCriteriosBusquedaDocumentoPagoListarPago(response) {
