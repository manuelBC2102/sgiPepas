...
function getResponseRegistroProveedorForm(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerParametrosInicialesPrincipal':
                debugger;
            ...
            case 'obtenerDocumentosAdministracionAsociativo':
                debugger;
    ...
    defaultData.forEach((item, index) => {
        ...
        tableHTML += `
            <tr>
                ...
                <td style="text-align:left;"><input type="number" value="${item['latitud'] }" class="form-control" placeholder="X" required /></td>
                <td style="text-align:left;"><input type="number" value="${item['longitud'] }" class="form-control" placeholder="Y" required /></td>
...
function obtenerParametrosIniciales() {
    ...
    ax.setAccion("obtenerParametrosInicialesPrincipal");
...
function onResponseObtenerParametrosIniciales(data) {
    ...
    $("#txtSector").val(data.datos[0].ubigeo);
    $("#txtUbicacion").val(data.datos[0].direccion);
    ...
    $("#secretNivel").val(data.datos[0].nivel);
    var rutaArchivo='/sgiLaVictoria/vistas/com/persona/firmas/'+data.datos[0].firma_digital;
    actualizarImagenFirma(rutaArchivo);
    // if(data.datos[0].nivel==2){
    //     document.getElementById('administracion').style.display = 'none';
    // }
    // if(data.datos[0].nivel==1){
    ...
    select2.cargarSeleccione("cboPlantas", data.plantas, "id", ["codigo_identificacion", "nombre"], "Seleccione Planta");
...
function actualizarImagenFirma(nuevaImagenUrl) {
    // Obtén el elemento <span> donde se encuentra la imagen
    var piePagFirma = document.getElementById('PiePagFirma');
    
    // Verifica que el elemento existe
    if (piePagFirma) {
        // Cambia el contenido del <span> para mostrar la nueva imagen
        piePagFirma.innerHTML = "<img src='" + nuevaImagenUrl + "' class='img-responsive' style='width: 170px; height: 68px'>";
    } else {
        console.error("El elemento con id 'PiePagFirma' no fue encontrado.");
    }
}
...
function obtenerDocumentosAdministracion(){
    debugger;
    var persona = trim(document.getElementById('secretPersona').value);
    ax.setAccion("obtenerDocumentosAdministracionAsociativo");
    ax.addParam("persona", persona);
...
function mensajeCulminado() {
    ...
    swal({
        ...
        type: "warning",
        text: "La invitación ya fue registrada y enviada aprobación",
...
function confirmAprobar() {
    ...
    swal({
        ...
        if (isConfirm) {
            finalizarAprobacion(nivel,invitacion);
            aprobar();
        }
    });
}

function finalizarAprobacion(nivel,invitacion){
    ax.setAccion("finalizarAprobacionAsociativo");
...
//     swal({
//         title: "¿Estás seguro?",
//         text: "¿Deseas rechazar esta invitación?",
//         type: "warning",
//         showCancelButton: true,
...
function rechazar(comentario) {
    ...
    swal({
        ...
        if (isConfirm) {
            finalizarRechazo(comentario,nivel,invitacion);
            sessionStorage.setItem('needsReload', 'true');
            // Volver a la página anterior
            window.history.back();
        } else {
            soloRechazar(comentario,nivel,invitacion);
            sessionStorage.setItem('needsReload', 'true');
            // Volver a la página anterior
            window.history.back();
        }
...
function finalizarRechazo(comentario,nivel,invitacion){
    ax.setAccion("finalizarRechazoAsociativo");
    ax.addParam("nivel", nivel);
...
function soloRechazar(comentario,nivel,invitacion){
    ax.setAccion("solicitarActualizacionAsociativo");
    ax.addParam("nivel", nivel);
    ax.addParam("comentario", comentario);
    ax.addParam("invitacion", invitacion);
    ax.consumir();
}
// Función de utilidad para trim
function trim(str) {
