...
function listarSubdiarios(){
    ax.setAccion("listarSubdiarios");  
...
function exitoSubdiarios(response) {
    if (response.status === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'listarSubdiarios':
                onResponseListarSubdiarios(response.data);
                onResponseVacio();
                loaderClose();
                break;
            case 'cambiarEstado':
            ...
            case 'eliminar':
                ...
                if (error > 0) {
                    swal("Eliminado!", "El subdiario: " + response.data['0'].descripcion + ".", "success");
                    listarSubdiarios();
                } else {
                    swal("Cancelado", "El subdiario: " + response.data['0'].descripcion + ". " + response.data['0'].vout_mensaje, "error");
                }
...
function nuevo() {
    var titulo = "Nuevo";
    var url = URL_BASE + "vistas/com/contabilidadSubdiarios/subdiarios_form.php?winTitulo=" + titulo;
    cargarDiv("#window", url);
}

function onResponseListarSubdiarios(data) {
    ...
    var cabeza = "<table id='datatable' class='table table-striped table-bordered'>"
            ...
            + "<th style='text-align:center; vertical-align: middle;'>Tipo asiento</th>"
    ...
    if (!isEmpty(data)) {
        $.each(data, function (index, item) {
            ...
            cuerpo_acc = cuerpo_acc 
                    ...
                    + "<a href='#' onclick='confirmarEliminar(" + item.id + ",\"" + item.subdiario_descripcion + "\")'><i class='fa fa-trash-o' style='color:#cb2a2a;'></i></a>&nbsp;\n";

            cuerpo = "<tr>"
                    + "<td style='text-align:left;'>" + item.subdiario_codigo + "</td>"
                    ...
                    + "<td style='text-align:left;'>" + item.tipo_asiento_descripcion+ "</td>"
            ;
...
function editar(id) {
    var titulo = "Editar";
    var url = URL_BASE + "vistas/com/contabilidadSubdiarios/subdiarios_form.php?winTitulo=" + titulo + "&id=" + id;
    cargarDiv("#window", url);
}
...
function confirmarEliminar(id, nom) {
    ...
    swal({
        title: "Estás seguro?",
        text: "Eliminarás el subdiario: " + nom + "!",
        type: "warning",
