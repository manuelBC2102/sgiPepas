...
function successPersona(response) {
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerConfiguracionesSolicitud':
                    onresponseConfiguracionesPersona(response.data);
                    dataPersonaGlobal = response.data.personaNatural;
                    //console.log(dataPersonaGlobal);
                    break;
            case 'insertAprobacionSolicitud':
                mostrarOk("Solicitud Aprobada");
                loaderClose();
                habilitarBoton();
                cargarListarPersonaCancelar();
                break;

            case 'insertDesaprobacionSolicitud':
                mostrarOk("Solicitud Desaprobada");
                loaderClose();
                habilitarBoton();
                cargarListarPersonaCancelar();
            break;
...
;
        ...
        select2.cargar("cboVehiculo", data.vehiculos, "id", "placa");
         select2.cargar("cboZona", data.zonasTodas, "id", "nombre");
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/solicitudRetiro/aprobacion_solicitud_listar.php', 'Aprobación de solicitud');
}
...
;
    ...
    swal({
        ...
        if (isConfirm) {
            loaderShow(null);
            deshabilitarBoton();
            ax.setAccion("insertAprobacionSolicitud");
            ax.addParamTmp("id", commonVars.personaId);
            // ax.addParamTmp("motivo", motivo);  // Enviar el motivo al controlador
            ax.consumir(); 
        } else {
            swal("Cancelado", "El registro fue cancelado", "error");
        }
...
function rechazarSolicitud() {
    swal({
        title: "¿Está seguro de desaprobar la Solicitud de retiro?",
        text: "",
        type: "info",
        showCancelButton: true,
        confirmButtonColor: "#33b86c",
    ...
    function(isConfirm) {
        if (isConfirm) {
            // Mostrar un segundo swal para ingresar el motivo
            swal({
                title: "Motivo de la desaprobación",
                text: "Escriba el motivo aquí...",
                type: "input",
                showCancelButton: true,
                confirmButtonColor: "#33b86c",
            ...
            function(motivo) {
                if (motivo === false) return false; // El usuario canceló
                if (motivo.trim() === "") {
                    swal.showInputError("Debe ingresar un motivo");
                    return false;
                }
                
                // Motivo ingresado correctamente
                loaderShow(null);
                deshabilitarBoton();
                ax.setAccion("insertDesaprobacionSolicitud");
                ax.addParamTmp("solicitud_id", commonVars.personaId);
                ax.addParamTmp("motivo", motivo);  // Enviar el motivo al controlador
                ax.consumir();

                swal("Enviado", "La solicitud de desaprobación ha sido enviada.", "success");
            });
        } else {
            swal("Cancelado", "La desaprobación fue cancelada", "error");
        }
    });
}
