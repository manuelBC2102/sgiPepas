...
$(document).ready(function () {
    ...
    ax.setAccion("obtenerConfiguracionesInvitacionTransportista");
    ...
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'obtenerPersona':
                ...
                break;
            case 'obtenerConfiguracionesInvitacionTransportista':
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/invitacion/invitacion_transportista_listar.php');
}
...
debugger;
    ...
    $('#txtReinfo').val(data.invitacion[0].codigo_identificacion).prop('readonly', true);
    $('#txtCodigo').val(data.invitacion[0].certificado).prop('readonly', true);
    $('#txtModalidad').val(data.invitacion[0].modalidad).prop('readonly', true);
 
    $('#txtUbicacion').val(data.invitacion[0].ubicacion).prop('readonly', true);
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/invitacion/invitacion_transportista_listar.php', 'Registro invitación');
}
...
function validarSolicitud(telefono,correo) {
...
function setearInputREINFO() {
    ...
    loaderShow();
    ax.setAccion("obtenerDataTransportista");
...
function guardarSolicitud() {
    ...
    var modalidad = trim(document.getElementById('txtModalidad').value);
    ...
    if (validarSolicitud(telefono,correo)) {
    
    loaderShow();
    if(invitacionId>0){
        ax.setAccion("actualizarInvitacionTransportista");
        ...
        ax.addParamTmp("modalidad", modalidad);
        ax.addParamTmp("telefono", telefono);
        ax.addParamTmp("correo", correo);
        ax.consumir();
    }else{
    ax.setAccion("guardarInvitacionTransportista");
    ax.addParamTmp("ruc", ruc);
    ax.addParamTmp("codigo", codigo);
    ax.addParamTmp("nombre", nombre);
    ax.addParamTmp("direccion", direccion);
...
function listarDataSolicitudes(data) {
    ...
    if(data==false){
        ...
        if (!isEmpty(data)) {
            ...
            $('#txtrazon').val(data.nombre).prop('disabled', true);
            $('#txtCodigo').val(data.codigo).prop('disabled', true);



        $('#txtDireccion').val(data.direccion).prop('readonly', true);
        $('#txtEstado').val(data.estado).prop('readonly', true);
        $('#txtModalidad').val(data.modalidad).prop('readonly', true);

        document.getElementById('env').disabled = false;
        loaderClose(); 
        swal("Validación Exitosa", "El transportista  está 'VIGENTE'.", "success");
    }
     else {
        // swal({
        //     title: "Error",
        ...
        //     icon: "error",
        // });
        swal("Error validación", "El transportista no se encuenta registrado en página MTC.", "error");
        $('#txtrazon').val('');
        $('#txtCodigo').val('');
        $('#txtDireccion').val('');
        $('#txtEstado').val('');
