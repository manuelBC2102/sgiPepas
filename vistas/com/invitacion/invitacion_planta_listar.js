...
//            $('#listaPersonaTipo').append('<li><a data-toggle="modal" data-target="#accordion-modal" onclick="cargarFormularioPersona(' + value.id + ',\'' + value.descripcion + '\',\'' + value.ruta + '\')">' + value.descripcion + '</a></li>');
        ...
        html='<div class="input-group-btn dropdown">'+
                '<button type="button" class="btn btn-info"  onclick="cargarFormularioPersona(' + perJuridica.id + ',\'' + perJuridica.descripcion + '\',\'' + perJuridica.ruta + '\')"><i class=" fa fa-plus-square-o"></i>&nbsp; Nueva invitación </button>'+
...
{    commonVars.personaId = 0;
    cargarDiv('#window', 'vistas/com/invitacion/invitacion_planta_form.php', "Nueva ");
}
...
//    var clasePersona = $("#cboClasePersonaBusqueda").val();
    ax.setAccion("getDataGridInvitacionPlanta");
     if(id==null){
                ...
                render: function (data, type, row) {
                    if (type === 'display') {
                        ...
                        return '<a onclick="editarInvitacion(' + row.id + ')"><b><i class="fa fa-edit" style="color:#088a08;"></i><b></a>\n\
                                   ...
                                   <a target="_blank" href="envio_notificacion5.php?token='+row.token+'"><b><i class="fa fa-file-pdf-o" style="color:#0366b0;"></i><b></a>\n\
...
function openModal(id,token) {
    ...
    var archivo = "/sgiLaVictoria/envio_notificacion5.php"; // Cambia esto por el nombre del archivo que necesites
    ...
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'ExportarPersonaExcel':
                ...
                break;
            case 'buscarCriteriosBusquedaSolicitudTransportista':
...
{    
    loaderShow();
    ax.setAccion("deleteSolicitudPlanta");
    ax.addParamTmp("id", id);
    ax.consumir();
}
...
function editarInvitacion(id) {
    ...
    cargarDiv("#window", "vistas/com/invitacion/invitacion_planta_form.php", "Editar Invitación ");
}
...
//    loaderShow();
    ax.setAccion("buscarCriteriosBusquedaSolicitudTransportista");
