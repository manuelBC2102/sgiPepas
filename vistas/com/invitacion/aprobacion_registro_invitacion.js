...
function successZona(response) {
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'listarInvitacionTransportista':
...
function listarInvitaciones() {
    debugger;
    ax.setAccion("listarInvitacionTransportista");
    ax.consumir();
}
...
function onResponseAjaxpGetDataGridZona(data) {
    ...
    var cabeza = '<table id="datatable" class="table table-striped table-bordered"><thead>' +
        ...
        "<th style='text-align:center;'>Código derecho</th>" +
        ...
        if (!isEmpty(data)) {
            ...
            $.each(data, function (index, item) {
                  ...
                  cuerpo = "<tr>" +
                    ...
                    "<td style='text-align:start;'>" + item.certificado + "</td>" +
                    
                    "<td style='text-align:start;'>" + item.ciudad_inscripcion + "</td>" +
                    "<td style='text-align:start;'>" + item.nivel + "</td>" +
                    "<td style='text-align:start;'>" + item.usuario + "</td>" +
                    "<td style='text-align:center;'>" +
                  
                    "<a  href='envio_notificacion4.php?token="+item.token+"'><b><i class='fa fa-file-pdf-o' style='color:#0366b0;'></i><b></a>\n" +
