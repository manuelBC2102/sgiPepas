...
function llenarcomboBancos() {
    ax.setAccion("obtenerBancos");
    ...
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerDataREINFO':
                ...
                loaderClose();
                break;
            case 'obtenerDataDNI2':
                listarDataSolicitudesDNI(response.data);
                loaderClose2();
                    break;
                
                case 'obtenerBancos':
                    onResponsellenarcomboBancos(response.data);
                    break;
        }
    } else
    {
        switch (response[PARAM_ACCION_NAME]) {
            ...
            case 'obtenerDataPlaca':
                ...
                break;
                case 'obtenerBancos':
                    mostrarMensajeError("No existen cuentas registradas");
                    deshabilitarBoton2;
                        break;
                        case 'obtenerDataDNI2':
                            mostrarMensajeError("Tuvimos un error al obtener la Data del minero");
                            loaderClose2();
                                break;
        }
    }
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/invitacion/invitacion_secundario_listar.php');
}

function onResponsellenarcomboBancos(data) {
    // Limpiar el select antes de agregar nuevas opciones
    $('#cboCuenta').empty();
    
    // Agregar la opción nula al principio
    $('#cboCuenta').append(new Option("Seleccione una cuenta", "", true, true)); // Esta opción será seleccionada por defecto

    // Llenar el select2 con las placas obtenidas en "data"
    $.each(data, function(index, item) {
        $('#cboCuenta').append(new Option(item.descripcion, item.id)); // Usamos "item.placa" como texto y "item.id" como valor
    });

    // Actualizar el select2 para reflejar los cambios
    $('#cboCuenta').trigger('change');
}
...
{
    loaderShow(null);
    cargarDiv('#window', 'vistas/com/invitacion/invitacion_secundario_listar.php', 'Registro invitación');
}
...
function validarSolicitud(telefono,correo,cuenta,numeroCuenta,cci) {
...
function guardarSolicitud() {
    ...
    var firma = document.getElementById('signatureData').value;
    var codigo = trim(document.getElementById('txtCodigoS').value);
    var nombre = trim(document.getElementById('txtResponsableNombre').value);
    var lugarNacimiento = trim(document.getElementById('txtLugarN').value);
    var fechaNacimiento = trim(document.getElementById('txtfechaN').value);
    var direccion = trim(document.getElementById('txtDireccionA').value);
    var estadoCivil = trim(document.getElementById('txtEstadoC').value);
    var hijo = trim(document.getElementById('txtHijos').value);
    var estatura = trim(document.getElementById('txtEstatura').value);
    var madre = trim(document.getElementById('txtMadre').value);
    var padre = trim(document.getElementById('txtPadre').value);
    var restriccion = trim(document.getElementById('txtRestriccion').value);
    ...
    var numeroCuenta = trim(document.getElementById('txtNumeroCuenta').value);
    var cci = trim(document.getElementById('txtCCI').value);
    if (validarSolicitud(telefono,correo,cuenta,numeroCuenta,cci)) {
    ...
    ax.setAccion("guardarComunero");
    ax.addParamTmp("dni", dni);
    ax.addParamTmp("tipo", tipo);
    ax.addParamTmp("foto", foto);
    ax.addParamTmp("codigo", codigo);
    ax.addParamTmp("nombre", nombre);
    ax.addParamTmp("lugarNacimiento", lugarNacimiento);
    ax.addParamTmp("fechaNacimiento", fechaNacimiento);
    ax.addParamTmp("direccion", direccion);
    ax.addParamTmp("estadoCivil", estadoCivil);
    ax.addParamTmp("hijo", hijo);
    ax.addParamTmp("estatura", estatura);
    ax.addParamTmp("madre", madre);
    ax.addParamTmp("padre", padre);
    ax.addParamTmp("restriccion", restriccion);
    ax.addParamTmp("sexo", sexo);
    ax.addParamTmp("telefono", telefono);
    ax.addParamTmp("correo", correo);
    ax.addParamTmp("cuenta", cuenta);
    ax.addParamTmp("numeroCuenta", numeroCuenta);
    ax.addParamTmp("cci", cci);
    ax.addParamTmp("firma", firma);
    ax.consumir();
    }  }
}
