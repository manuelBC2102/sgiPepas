...
function cargarBienDetalleCombo(data, indice, valorInicial) {
    ...
    if (!isEmpty(data)) {
        ...
        if (isEmpty(valorInicial)) {
            ...
            $("#cboBien_" + indice).append('<option value="' + valorInicial.id + '">' + valorInicial.text + '</option>');
        ...
        switch (item.tipo) {
            ...
            case 11:
                ...
                if (item.codigo == '9') {
                    ...
                    if(hanPasadoMasDeUnDia(camposDinamicos[index]["valor"], fechaActual)){
                        mostrarValidacionLoaderClose("Fecha de emisión no debe ser menor más de 1 día: " + item.descripcion);
                        isOk = false;
                        return false;
                    }
                }
...
//        $('#dtBodyDocumentoGenerado').append(cuerpo);
        ...
        if (!isEmpty(data.dataPlantilla)) {
            ...
            if (!isEmpty(dataImp.dataDocumento)) {
                if(dataImp.dataDocumento[0].documento_tipo_id != "12"){
                    cargarDatosImprimir(dataImp, 1);
                }else{
                    if (!isEmpty(data.resEfact) && data.resEfact.esDocElectronico == 1) {
