...
function asignarImporteDocumento() {
    ...
    if (!isEmpty(importes.calculoId)) {
        ...
        document.getElementById(importes.subTotalId).value = devolverDosDecimales(calculo.afecto);
        igv = Math.round(igvValor * calculo.afecto);
...
//        document.getElementById(importes.noAfectoId).value = devolverDosDecimales(calculo.noAfecto);
        document.getElementById(importes.totalId).value = devolverDosDecimales(calculo.afecto + igv + calculo.noAfecto);
...
function cargarModalCopiarDocumentos() {
    ...
    setTimeout(function(){ cambiarAnchoBusquedaDesplegable(); }, 100);
}
...
function cerrarModalEar(){
    $('#modalEar').modal('hide');
    setTimeout(function(){ cargarPantallaListar();}, 200);  
}
