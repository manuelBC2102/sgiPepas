...
function guardarSolicitud() {
    ...
    var ubicacion = trim(document.getElementById('txtUbicacion').value);
...
function listarDataSolicitudes(data) {
    ...
    if (data[0].estado === 'VIGENTE') {
        ...
        $('#txtCodigoUnico').val(data[0].codigo_anterior).prop('readonly', true);
        $('#txtNombre').val(data[0].nombre).prop('readonly', true);
        $('#txtSector').val(data[0].sector).prop('readonly', true);
        $('#txtEstado').val(data[0].estado).prop('readonly', true);
        var ubicacion = data[0].departamento + '-' + data[0].provincia + '-' + data[0].distrito;
        $('#txtUbicacion').val(ubicacion).prop('readonly', true);
