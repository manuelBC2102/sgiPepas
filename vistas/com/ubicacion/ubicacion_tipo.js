...
{
    document.getElementById("frm_ubicacion_tipo").reset();
}

function validar_ubicacion_tipo_form() {
    var bandera = true;
    var espacio = /^\s+$/;
...
function cargarDivGetUbicacionTipo(id) {
    $("#window").empty();
    cargarDiv("#window", "vistas/com/ubicacion/ubicacion_tipo_form.php?id=" + id + "&" + "tipo=" + 1);
}

function listarUbicacionTipo() {
    breakFunction();
    acciones.iniciaAjaxTest(COMPONENTES.UBICACION, "successUbicacion");
    ax.setAccion("getDataGridUbicacionTipo");
    ax.consumir();
}
...
function successUbicacion(response) {
    if (response['status'] === 'ok') {
        switch (response[PARAM_ACCION_NAME]) {
            case 'getDataGridUbicacionTipo':
                onResponseAjaxpGetDataGridUbicacionTipo(response.data);
                $('#datatable').dataTable({
                    "language": {
            ...
            case 'cambiarTipoEstado':
                ...
                break;
            case 'deleteUbicacionTipo':
                var error = response.data['0'].vout_exito;
                if (error > 0) {
                    swal("Eliminado!", " " + response.data['0'].nombre + ".", "success");
                } else {
                    swal("Cancelado", "Upss!!. " + response.data['0'].nombre + " " + response.data['0'].vout_mensaje + " en el mantenedor ubicaciones", "error");
                }
...
function exitoUpdate(data)
{
    ...
    else
    {
        ...
        cargarDiv("#window", "vistas/com/ubicacion/ubicacion_tipo_listar.php");
    }
}
...
function onResponseAjaxpGetDataGridUbicacionTipo(data) {
    var cuerpo_total = '';
    var cuerpo = '';
    ...
    $.each(data, function (index, item) {
        ...
        cuerpo = "<tr>" +
                ...
                "<a href='#' onclick='cargarDivGetUbicacionTipo(" + item.id + ")'><b><i class='fa fa-edit' style='color:#E8BA2F;'></i><b></a>&nbsp;\n" +
                "<a href='#' onclick='confirmarDeleteUbicacionTipo(" + item.id + ", \"" + item.descripcion + "\")'><b><i class='fa fa-trash-o' style='color:#cb2a2a;'></i><b></a>" +
                "</td>" +
                "</tr>";
...
function guardarUbicacionTipo(tipo)
{
    ...
    if (tipo == 1)
    {
        updateUbicacionTipo(id, descripcion, codigo, comentario, estado, empresa, comboT);
    } else {
        insertUbicacionTipo(descripcion, codigo, comentario, estado, usu_creacion, empresa, comboT);
    }
}

function insertUbicacionTipo(descripcion, codigo, comentario, estado, usu_creacion, empresa, comboT)
{
    if (validar_ubicacion_tipo_form()) {
        deshabilitarBoton();
        ax.setAccion("insertUbicacionTipo");
...
function getUbicacionTipo(id_ubi_tipo)
{
    ax.setAccion("getUbicacionTipo");
    ax.addParamTmp("id_ubi_tipo", id_ubi_tipo);
    ax.consumir();
}
...
function updateUbicacionTipo(id, descripcion, codigo, comentario, estado, empresa, comboT)
{
    if (validar_ubicacion_tipo_form()) {
        deshabilitarBoton();
        ax.setAccion("updateUbicacionTipo");
        ax.addParamTmp("id_ubi_tipo", id);
        ax.addParamTmp("descripcion", descripcion);
        ax.addParamTmp("codigo", codigo);
        ax.addParamTmp("comentario", comentario);
        ax.addParamTmp("estado", estado);
...
function confirmarDeleteUbicacionTipo(id, nom) {
...
function deleteUbicacionTipo(id_ubi_tipo, nom)
{
    ax.setAccion("deleteUbicacionTipo");
    ax.addParamTmp("id_ubi_tipo", id_ubi_tipo);
